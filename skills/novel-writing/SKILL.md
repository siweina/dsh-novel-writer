...
- 返回：`anchor`（上一章结尾原文，即**承接口**）｜`previousHook`｜`outlineDirection`｜`characters`｜`openPlots`（每条带 `distance` = 埋了多久没回收）｜`worldview`｜`baseline`（六维 μ/σ/容差）｜`anchors` + `skeletons`（照这个味道与形状写）｜`lastVerdict`｜`avoid`（**本章禁用清单**：已回收伏笔 ∩ 本章方向行的关键词、世界观禁词、已退场角色）｜`plan.checklist`｜`degraded`。
- **只读**：不写任何文件（仅复用既有分析缓存）。
...
系统提示词由**两个正交维度**决定，都在侧边栏「写作助手功能」面板调整，也可用 `novel_sentence_config` 读写：
...
- **向后兼容**：`general` 就是 4.x 的那段通用工作流，没有 `promptScene` 键的老状态文件自动落到 `general`，**既有工作流不变**（本版只在其中补入三条新工具的入口指引，共 +7 行，其余逐字沿用 4.x）。
- 场景只改变注入的提示词内容，**不影响任何工具的能力**。
