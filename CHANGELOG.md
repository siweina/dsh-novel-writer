...
用户反馈：侧边栏「写作助手功能」面板里，「提示词场景」那一行点不了其他选项。

**原因**：`lib/client.js` 把五个场景按钮的 `disabled` 绑在 `systemPromptMode !== "full"` 上——而默认档位是 `off` / `brief`，于是场景行整行置灰、五个按钮全不可点，**且没有任何文案说明"要先把档位切到完整"**——等于把一个功能锁在一个看不见的前提后面。

**修法**：
- 场景按钮**始终可点**（只保留 `state.loading` 这一个禁用条件），与档位行保持一致；
- 档位非 `full` 时，在场景行说明里追加一句（中英双语）：「当前档位不是「完整」，场景已保存但暂不注入——把上面的档位切到「完整」即生效。」——用户仍可先选场景，切档后自动生效；
- 语义不变：场景值仍只在 `full` 档真正注入。

**回归测试**：`test/client-test.mjs` 的 vnode 桩新增**按钮收集**（此前只统计节点数与类名，按钮从未被断言——所以这个一眼可见的问题测试完全看不见），并新增一段断言：档位=`off` 且 `loading=false` 时，档位 3 个 + 场景 5 个分段按钮**全部可点**、点击「写新章」真的回调 `toggle({ promptScene: "writing" })`、当前场景高亮正确。**该断言在修复前必然失败**（实测已复现）。

## [5.1.0] - 2026-09-17

**成本与僵化修复：把 v5.0.0 里被写成"每次必做"的可选动作，改回按需。**

用户反馈：插件反复调用工具、步骤明显变多；**改稿比单写还贵**（约 0.2~0.3 元/万字 → 0.7 元）；写出来的文章**整体僵化**。排查结论：**不是崩溃、也不是死循环**，而是三处设计把"可选"写成了"必做"——场景提示词里的强制步骤链、改稿台的逐条处理循环、以及默认过长的改稿清单；叠加上"照骨架造句"与"任一维度出带必改"两处口径，就同时造成了贵与僵。

**兼容性**：工具数量与返回结构不变（仍 18 个；`items[]` 仍 9 字段）；新增状态字段 `leanWorkflow`（默认 `false`，老状态文件行为不变）；`DEFAULT_MAX_ITEMS` / `MAX_PARAGRAPH_ITEMS_PER_METRIC` 是内部常量，`novel_fix_plan` 从未向调用方暴露 `maxItems`。

### 一、成本：三处「必做」改「按需」（P0）

| 位置 | v5.0.0 旧文案 | v5.1.0 新文案 |
|---|---|---|
| 场景 `writing` ⑤⑥ | 「用 novel_new_chapter 建章节文件，然后**【强制】调用 novel_style_check**」「**收尾三件**：novel_plot add / novel_outline hook / novel_summary」 | 「写完**建议**跑一次自检，只有相似度明显偏低或某维偏离超过 2 倍容差时才修正」「收尾**按需**，不必每章全套」 |
| 场景 `revising` ②④⑤ | 「按 severity **逐条处理**」「**每改完一批就登记并复测**：mark → verify…只继续处理 pending 与 new」「全部处理完再跑一次 style_check」 | 「**分批改**：同一段落/同一类型合并成一次修改，一轮只处理最严重的 3~5 条」「**不要逐条 mark，也不要反复 verify**：一轮改完只跑一次 verify」 |
| 通用档（4.x 沿用） | 「动笔前**必须**调用 novel_style_report」「**每写完一章必须**调用 novel_style_check…任一维度出带时逐句修正」 | 「基线由开写包/建章直接附带，**不必专门去调工具**」「**建议**跑一次自检；只有偏离明显时才修正」 |
| `novel_new_chapter` 渲染 | 【风格基线强制流程】①…②必须调用 style_check… | 【基线怎么用】① 锚段读一遍校准语感（不要照 skeletons 造句）② **建议**自检、偏离明显才改 |
| 开写包 `plan.checklist` | 「跑 novel_style_check 自检」「写完回填钩子」 | 「**建议**自检；单一维度轻微出带属正常波动」「收尾按需」 |
| 改稿台清单渲染 | 无处理建议 | 新增「**分批改**，一轮 3~5 条；不必逐条 mark，改完最多 verify 一次」 |

### 二、清单规模（P1）

- `MAX_PARAGRAPH_ITEMS_PER_METRIC` **3 → 1**：同一维度只报最严重的 1 段（其余由 summary 计数体现）
- `DEFAULT_MAX_ITEMS` **40 → 12**：清单是"先改最要紧的"，不是"一次改完"
- 指标类 hint 追加软化说明（仅 `abstractDensity` / `gapIndex` / `hedgeDensity` 三类）：「仅当这里读起来确实别扭才改；抒情、心理、留白段落属正常写法，可标记 skip 保留。」hint 前缀格式未动，便于对照与测试
- 实测（同一故意写坏的章）：**旧默认 13 条 → 新默认 6 条**；正常章仍 **0 条**（无误报）

### 三、僵化：去掉"为对齐数字改文"的驱动力（P0）

1. **「照骨架造句」改口**：原先在 4 处重复的「按 anchors/skeletons 的味道与形状写」收敛为——**anchors 只用来校准语感，不要照 skeletons 造句、不要套句式模板**（骨架仅在写不出该类型句子时参考）
2. **取消「任一维度出带必改」**：改为「只有偏离达到 2 倍容差以上、或那一处读起来确实别扭时才改」。理由写进文案：**1.5σ 带宽下六维里至少一维出带是常见现象**，正常波动不该被当成缺陷反复打磨
3. **指标类待办要判断再动手**：抒情 / 心理 / 留白 / 节奏变化本来就是正常写法，读起来不别扭就保留（可用 `mark` 标 `skip` 记一笔）
4. `novel_style_check` 的 `advice` 与 `novel_chapter_brief` 的清单同步改写：`diffs` 与六维两段都不再要求"逐句修正"，先问"这是不是有意为之"
5. **同一份输出里的自相矛盾修掉**（装机实测时发现）：六维汇总行由「N 个维度超出容差带，**需修正**」改为「N 个维度超出容差带**（先判断是否为有意为之，偏离明显或读起来别扭才改）**」（`lib/style-metrics.js` 的 `judge.summary`）；渲染层删掉句尾的「——请对照基线修正后再续写」——它与紧随其后的建议段直接冲突，而模型会听更早、更硬的那半句

### 四、精简工作流开关（P2，新增）

侧边栏「写作助手功能」面板新增 **精简工作流**（英文 `Lean workflow`），状态字段 `leanWorkflow`（默认 `false`）：

- 打开后系统提示词注入 **`LEAN_TEXT`（380 字符）**，**优先于档位与场景**：只保留"工具按需调用"一条原则，去掉自检/复测/登记的全部催促
- `novel_style_report` 与 `novel_sentence_analysis` 在调用方**未显式指定 `brief`** 时默认走精简输出（前者实测省 2k+ 字符/次）
- 与「提示词场景」不同：**任何时候都能切换**，不受 `full` 档限制；非法值静默忽略，缺省回退 `false`
- **模型侧同样可用**：`novel_sentence_config` 的输入/输出 schema 与渲染文本都带上 `leanWorkflow`（模型能查、能设，不必只靠侧边栏 GUI）；实测精简工作流下 `novel_style_report` 未传 `brief` 也返回简报（**181 字符 vs 完整 1725 字符**），显式 `brief=false` 时仍返回完整报告

### 五、验证

- 五套自带测试全部通过：**unit 20 项 / pattern / client / e2e（ALL PASSED）/ MCP 协议 67 项**
- e2e 同步更新：`new_chapter` 渲染断言由「强制流程」改为「基线怎么用」；改稿台的"正常章 0 项""指标类分支仍覆盖段落归因""落盘条数与返回值一致"等断言在新默认值下均保持成立（Agent A 复核确认无需改动其余断言）
- 成本复测（同一夹具、同一流程，见下表）

### 六、成本对照（一章 1 万字的模型开销模型）

| 环节 | v5.0.0 | v5.1.0 |
|---|---|---|
| 写一章固定调用 | **6 次**（brief + new_chapter + style_check + plot add + hook + summary） | **2 次起**（brief + new_chapter）；自检与收尾按需 |
| 改一章清单规模 | 16 条 / **5073 字符**（511 字小章实测） | 6 条 / **约 1.9k 字符**（同一坏章） |
| 改一章调用数 | 清单条数 × 2 + verify + style_check ≈ **30+ 次** | 分批 2~4 次编辑 + 1 次 verify（用户要求时） |
| 每请求固定开销 | 工具目录 13,263 字符 + 通用档 3,047 字符 | 同左（工具数未变）；精简工作流下提示词 **380 字符** |
