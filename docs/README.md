...
> **v5.1.1 已于 2026-09-18 发布**（站点与代码同一次推送）：工具页 JSON-LD `softwareVersion` → 5.1.1（18 页）；首屏品牌版本、Release 胶囊链接、`#trust` 的 `latest` 与根 `index.html` 的 JSON-LD `softwareVersion` 一并翻到 **5.1.1**（上一轮按"未发布"刻意停在 5.1.0，**本轮发布后翻正**）。本轮改动**只在客户端 UI 与测试**：面板「提示词场景」的五个按钮此前被 `disabled` 锁死（条件绑在 `systemPromptMode !== "full"` 上，而默认档位是 off/brief —— 用户反馈"点不了其他选项"），现改为**始终可点**，并在档位非「完整」时于行内追加中英双语说明「场景已保存但暂不注入——把上面的档位切到「完整」即生效」。**注入语义未变**（场景仍只在 `full` 档生效），文档站其余页面无需改内容。
