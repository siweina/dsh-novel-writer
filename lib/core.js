 ...
 * 本文件承载辅助函数、常量、HTTP 路由与状态读写；lib/index.js 仅保留插件协议与 18 个工具注册。
...
// v5.0.0：系统提示词的「场景」枚举由 prompts.js 单一持有（core.js 只做合法性校验与持久化）
import { PROMPT_SCENES } from "./prompts.js";
...
// v4.3.0：章号解析前的字符宽度归一——全角数字（０-９）与全角连字符类（－．／）映射成半角。
// 不用整串 NFKC：NFKC 会把标题里的「。」(U+3002) 一起归一成「.」，再被 cleanChapterTitle 的
// [_\-.]+ 换成空格，破坏标题外观（"初遇。重逢" → "初遇 重逢"）；这里只映射章号相关字符，1 字符换 1 字符，
// 索引与原文一一对应（bookNameFromFileName 依赖索引切片，故不能改长度）。
const CHAPTER_WIDTH_PAIRS = /[\uFF10-\uFF19\uFF0D\uFF0E\uFF0F]/g;
export function normalizeChapterWidth(text) {
  return String(text).replace(CHAPTER_WIDTH_PAIRS, (ch) => String.fromCharCode(ch.charCodeAt(0) - 0xfee0));
}
export function parseChapterNumber(name) {
  // v4.3.0 修正：入口先做宽度归一——旧版「第０１章 初遇.md」解析不出章号（number=undefined），
  // 可与「第01章.md」并存成为同一章并绕过重号防护（nextFreeChapterFile / novel_summary 归一化键都会失守）
  const stem = normalizeChapterWidth(name).replace(/\.[^.]+$/, "");
...
export function normalizeChapterKey(c) {
  // v4.3.0 修正：先做宽度归一（「第０１章」→「第01章」）——否则全角章号归一化失败，
  // novel_summary 会把全角章与半角章当成两个不同的槽位（重号防护被绕过）
  const s = normalizeChapterWidth(c ?? "").trim().replace(/\.md$/i, "").replace(/^#+\s*/, "");
  // v4.3.0 修正：字符类与 parseChapterNumber:141 对齐（补「千万」）。
  // 旧版缺「千」→「第一千零一章」会从「千」之后开始匹配，归一成「第01章」，
  // 与 parseChapterNumber=1001 冲突，导致 novel_summary 把第 1001 章摘要写进第 1 章（静默覆盖）。
  const m = s.match(/第?(\d{1,4}|[零一二三四五六七八九十百千万两]+)[章回话节]/);
  if (m) {
    let n;
...
export function cleanChapterTitle(stem) {
  // v4.3.0 修正：宽度归一（「第０１章 初遇」→「第01章 初遇」），全角章号不再混进标题
  return normalizeChapterWidth(stem)
    .replace(/第?(\d{1,4})[章回话]/g, "")
    .replace(/第[零一二三四五六七八九十百千万两]+[章回话]/g, "") // v4.3.0：补「千万」，与章号解析口径一致
    .replace(LEADING_NUMBER, "")
    .replace(/[_\-.]+/g, " ")
    .trim();
}
/**
 * v5.0.0 修正：从正文一级标题里取章节标题。
 *
 * 背景：`novel_new_chapter` 过去把 title 写进 Markdown 一级标题，文件名却固定是「第07章.md」；
 * 而章节标题一直只从**文件名**派生（cleanChapterTitle），于是用 title 建的章在 novel_chapters
 * 清单里显示成空标题（`第07章  —`）。这里做一次回退：文件名没有标题时读正文标题。
 * 只作为回退用——文件名有标题时一律以文件名为准（用户改文件名比改正文更直观）。
 * 一级标题若只是章号本身（「# 第07章」）则返回空串，不把章号当标题重复显示。
 */
export function firstHeadingTitle(text) {
  const m = String(text ?? "").match(/^[ \t]*#{1,6}[ \t]*([^\n]+)/m);
  if (!m) return "";
  const heading = normalizeChapterWidth(m[1]).replace(/\s+/g, " ").trim();
  return cleanChapterTitle(heading).slice(0, 60);
}
...
// v5.0.0：宿主契约要求长任务可取消。取消（AbortError）不是"数据取不到"，
// 它是调用方主动中止——任何 catch 降级分支都必须先用本函数放行，否则取消会被
// 伪装成"成功但内容为空"的结果返回。全插件唯一判据，勿在别处再写一份。
...
// v4.3.0：无 BOM UTF-16 探测用的两个判定。
// ① 高字节结构：UTF-16 码元的高字节决定大致区块——0x00 ASCII/拉丁、0x4E-0x9F CJK 统一表意、
//    0x30-0x33 CJK 标点与假名、0xFF 全角形式、0xD8-0xDF 代理对……（LE 的高字节在奇数下标，BE 在偶数）
function isTextHighByte(b) {
  return b === 0x00 || (b >= 0x01 && b <= 0x06) || (b >= 0x0e && b <= 0x33) || (b >= 0x4e && b <= 0x9f) || (b >= 0xd8 && b <= 0xdf) || b >= 0xfe;
}
// ② 试解后可打印率：GBK 字节被误读成 UTF-16 时典型落点是谚文/彝文/私用区，
//    这里只认真正的文本区（CJK/ASCII/常见标点/全角形式），用来把"结构像"和"内容像"同时卡住
function isPlausibleTextCode(cp) {
  if (cp === 0x09 || cp === 0x0a || cp === 0x0d) return true;
  if (cp >= 0x20 && cp <= 0x7e) return true; // ASCII 可打印
  if (cp >= 0x00a0 && cp <= 0x024f) return true; // 拉丁扩展
  if (cp >= 0x0370 && cp <= 0x04ff) return true; // 希腊/西里尔
  if (cp >= 0x2010 && cp <= 0x206f) return true; // 通用标点
  if (cp >= 0x20a0 && cp <= 0x20bf) return true; // 货币符号
  if (cp >= 0x2190 && cp <= 0x21ff) return true; // 箭头
  if (cp >= 0x2460 && cp <= 0x27bf) return true; // 带圈字符/框线/装饰符号
  if (cp >= 0x3000 && cp <= 0x303f) return true; // CJK 标点
  if (cp >= 0x3040 && cp <= 0x30ff) return true; // 平假名/片假名
  if (cp >= 0x3105 && cp <= 0x312f) return true; // 注音符号
  if (cp >= 0x3400 && cp <= 0x4dbf) return true; // CJK 扩展 A
  if (cp >= 0x4e00 && cp <= 0x9fff) return true; // CJK 统一表意
  if (cp >= 0xd800 && cp <= 0xdfff) return true; // 代理对（emoji 等）
  if (cp >= 0xf900 && cp <= 0xfaff) return true; // CJK 兼容表意
  if (cp >= 0xfe30 && cp <= 0xfe4f) return true; // CJK 兼容形式
  return cp >= 0xff00 && cp <= 0xffef; // 全角/半角形式
}
// v4.3.0：按某字节序试解，返回 { coherent 高字节结构占比, plausible 可打印率 }
function scoreUtf16(buf, le) {
  const units = buf.length >> 1;
  let coherent = 0;
  let plausible = 0;
  for (let i = 0; i + 1 < buf.length; i += 2) {
    const high = le ? buf[i + 1] : buf[i];
    const low = le ? buf[i] : buf[i + 1];
    if (isTextHighByte(high)) coherent += 1;
    if (isPlausibleTextCode(high * 256 + low)) plausible += 1;
  }
  return { coherent: coherent / units, plausible: plausible / units };
}
export function decodeTextBuffer(buf, filePath) {
  ...
  // 3b) v4.3.0 新增：中文正文的 UTF-16（尤其 LE）NUL 占比只有约 1.5%，过不了上面 5% 的阈值，
  //     旧版会一路落到 GBK 分支——要么解出乱码静默返回，要么把合法 UTF-16 报成"不是有效的 UTF-16"。
  //     这里用"高字节结构 + 试解可打印率"双阈值判定，且要求另一字节序明显不成立（避免 GBK 误判）
  let utf16Suspected = false;
  if (buf.length >= 8 && buf.length % 2 === 0) {
    const le = scoreUtf16(buf, true);
    const be = scoreUtf16(buf, false);
    const leWins = le.coherent + le.plausible >= be.coherent + be.plausible;
    const best = leWins ? { encoding: "utf-16le", score: le } : { encoding: "utf-16be", score: be };
    const other = leWins ? be : le;
    if (best.score.coherent >= 0.9 && best.score.plausible >= 0.9 && other.coherent < 0.9) {
      return new TextDecoder(best.encoding).decode(buf);
    }
    // 结构像 UTF-16 但可打印率不够：只作为报错文案的线索（不当成结论，避免误吞 GBK 文件）
    utf16Suspected = best.score.coherent >= 0.9 && best.score.plausible >= 0.6;
  }
  // 4) 中文 Windows 常见的 GBK/GB18030（TextDecoder 原生支持 gbk）
  const gbk = new TextDecoder("gbk").decode(buf);
  if (!gbk.includes("\uFFFD")) return gbk;
  // v4.3.0：区分"可能是缺 BOM 的 UTF-16"与"真正无法识别"——旧文案把合法 UTF-16 也一并说成"不是有效的 UTF-16"
  const hint = utf16Suspected
    ? "；该文件的高位字节分布像 UTF-16，可能是缺 BOM 的 UTF-16LE/BE——请另存为 UTF-8（UTF-16 请带 BOM）"
    : "，也不像带 BOM 或缺 BOM 的 UTF-16";
  throw new Error(`cannot detect text encoding of "${filePath}": 不是有效的 UTF-8/GBK${hint}，请将文件另存为 UTF-8 编码`);
}
export function countTextLines(text) {
  // v4.3.0 新增：行数口径的唯一出口——空文件（含仅 BOM 已剥离后的空串）= 0 行。
  // 起因：chapterStats 给 0 行而 novel_read（index.js）用 text.split(/\r?\n/).length 给 1 行（split 至少返回 [""]），
  // 同一章在两处显示不同行数。index.js 侧建议改用本函数（见报告"需父代理裁决"）。
...
export async function chapterStats(bookPath, chapter, exec) {
  ...
  return {
    chars: text.length,
    lines: countTextLines(text), // v4.3.0：口径统一（空文件 0 行），不再内联三元表达式
    size: info.size,
    updated: info.mtime.toISOString(),
    // v5.0.0 修正：正文一级标题（可为空串）。novel_chapters 在"文件名没有标题"时回退用它，
    // 否则用 novel_new_chapter title 建的章在清单里标题为空。正文已读在这里，零额外 IO。
    heading: firstHeadingTitle(text)
  };
}
export function findChapter(chapters, chapterArg) {
  const needle = String(chapterArg).trim();
  const lower = needle.toLowerCase();
  // v4.3.1 修正（P0）：精确文件名（含省略扩展名的文件名）**优先于**章号解析。
  // 旧版把章号匹配放在最前，于是「第03章 重逢.md」这种完整文件名也会被 parseChapterNumber 解析出章号 3，
  // 命中同章的另一个文件；被抢走的那一份用章号/文件名/标题三条路都读不到（永久不可达，实测复现）。
  const plain = lower.replace(/\.(md|markdown|txt)$/i, "");
  const byFile = chapters.find((c) => {
    const file = c.file.toLowerCase();
    return file === lower || file.replace(/\.[^.]+$/, "") === plain;
  });
  if (byFile) return byFile;
  const asNumber = parseChapterNumber(needle);
  if (asNumber !== void 0) {
    const matched = chapters.filter((c) => c.number === asNumber);
    // v4.3.1 修正（P0）：同章号多文件不再"静默取排序后的第一个"——那会让另一份永远读不到。
    // 改为拒绝并列出候选；候选文件名可直接回传给 chapter 参数（文件名匹配已在上方优先）。
    if (matched.length > 1) {
      throw new Error(
        `第 ${asNumber} 章有 ${matched.length} 个文件，无法确定要哪一份：` +
        matched.map((c) => c.file).join("、") +
        `。请把 chapter 参数写成其中一个文件名（可省略扩展名）来指定；` +
        `或用 novel_continuity_check 查看同章号清单后改名。`
      );
    }
    if (matched.length === 1) return matched[0];
  }
  // v4.0.0 修正：纯数字参数必须"章号完全相等"，否则 findChapter("1") 会子串命中第 10/11/12 章
  // v4.3.0 修正：这里原有一段 `chapters.find(c => String(c.number) === needle)` 的"精确查找"，
  // 恒为无效代码——走到此处说明没有任何章号的数值等于 asNumber，而 c.number 是 parseChapterNumber 产出的
  // Number，String(c.number) === needle 只可能对 needle="01" 之类成立，此时数值必然等于 asNumber（早退命中）。
  // 故删除该死查找；但 `return void 0` 必须保留（它是"纯数字不下探子串兜底"的承重守卫，
  // 删掉后 findChapter("1") 会经 byTitle 命中 "第11章.md"）。宽度归一是为了全角参数（"０１"）同口径。
  if (/^\d+$/.test(normalizeChapterWidth(needle))) return void 0;
...
export function nextFreeChapterFile(destDir, fileName) {
  ...
  const mCjk = /^(.*第)([零一二三四五六七八九十百千万两]+)([章回话].*)$/.exec(stem);
  if (mCjk) {
    let bump = next;
    let candidate = null;
...
export function bookNameFromFileName(fileName) {
  ...
  // v4.3.0 修正：章号标记必须有 章/回/话/节，或数字两侧是明确边界——旧版把 [章回话节] 写成可选，
  // 「三 体.md」在 index 0 就被命中（中文数字"三" + 空格）→ head 截成空串 → 书名被判为 undefined 整本丢失。
  // 宽度归一只用于匹配（1 字符换 1 字符，索引一一对应），书名仍按原 stem 切分，不改写书名里的全角字符。
  const stemN = normalizeChapterWidth(stem);
  const chapterMark = stemN.match(/第?\d{1,4}[章回话节]|第[零一二三四五六七八九十百千万两]+[章回话节]|(?:^|[\s._\-—~])\d{1,4}(?=[\s._\-—~]|$)/);
  const head = chapterMark ? stem.slice(0, chapterMark.index).trim() : stem;
  const parts = head
    .replace(/第?(\d{1,4})[章回话]/g, " ")
    .replace(/第[零一二三四五六七八九十百千万两]+[章回话]/g, " ")
    .replace(LEADING_NUMBER, " ")
    .split(/[\s._\-—~]+/)
    .map((p) => p.trim())
  ...
  // v4.3.0 修正：同 chapterMark——必须带 章/回/话/节，或数字带明确边界；「中文数字+空格」开头（三 体.md / 第一部.md）
  // 是书名而不是章号（旧的 [中文数字]+ + 分隔符 口径会把这类书名一并判成"章号开头"再按 parts.length<=2 丢掉）
  const startsWithChapter = /^第?\d{1,4}[章回话节]|^第[零一二三四五六七八九十百千万两]+[章回话节]|^\d{1,4}(?=[-._\s—])/.test(stemN);
...
export function bookNameFromContent(text) {
  ...
  for (const raw of lines) {
    ...
    const hasChapterMark = /第?(\d{1,4})[章回话]|第[零一二三四五六七八九十百千万两]+[章回话]|^\d{1,4}[-._\s]/.test(line);
...
// v5.0.0：leanWorkflow=精简工作流（全局布尔开关，默认 false=保持既有强制清单行为）。
// 它与 promptScene 同级（顶层键，不是 features 功能开关）——写进 features 会被 featureEnabled 的白名单语义污染。
export const SENTENCE_ANALYSIS_DEFAULTS = Object.freeze({ enabled: true, autoAnalyze: true, systemPromptMode: "brief", promptScene: "general", leanWorkflow: false });
export const ALL_TOOLS = Object.freeze([
  ...
  "novel_outline",
  // v5.0.0：写作能力层——开写包（动笔前装配材料）与改稿台（诊断转待办）
  "novel_chapter_brief", "novel_fix_plan"
]);
export const TOOL_LABELS = Object.freeze({
  ...
  novel_chapter_brief: "开写包", novel_fix_plan: "改稿台"
});
...
export function scanWordHits(text, words) {
  ...
  for (const word of sortedMarkerWords(words)) {
    ...
    // v4.3.0 修正：补空词条守卫——wlen===0 时 from = hit + 0 不推进，while 条件恒真 → 死循环。
    // 当前词表无空串，属"缺守卫"的潜在缺陷（词典/自定义词表一旦引入空串就是挂死）
    if (wlen <= 0) continue;
    let n = 0;
    let from = 0;
...
export async function readSentenceState() {
  try {
    ...
    return {
      ...
      // v5.0.0：提示词场景（与档位正交；老状态文件没有该键 → general，即 4.x 通用工作流 + 本版补入的 7 行新工具指引）
      promptScene: PROMPT_SCENES.includes(parsed.promptScene) ? parsed.promptScene : SENTENCE_ANALYSIS_DEFAULTS.promptScene,
      // v5.0.0：精简工作流（严格布尔校验：只有真正的 true/false 才采用，"true"/1/null/缺失一律回退 false，
      // 与上面 promptScene 的白名单校验同一风格——不做真值转换，避免老状态文件里的脏值把开关"意外打开"）
      leanWorkflow: typeof parsed.leanWorkflow === "boolean" ? parsed.leanWorkflow : SENTENCE_ANALYSIS_DEFAULTS.leanWorkflow,
...
export function readSentenceStateSync() {
  try {
    const file = stateFilePath();
    // v5.0.0：无状态文件/解析失败时仍给出 leanWorkflow —— 调用方（提示词注入侧）无条件读 st.leanWorkflow，
    // 不能出现"有文件=布尔值、无文件=undefined"两套口径（其余字段保持既有的"缺失即 undefined"语义不动）
    if (!existsSync(file)) return { leanWorkflow: false };
    ...
    return {
      ...
      // v5.0.0：提示词场景（同步版，供 systemPrompt.section 的 text 回调使用）
      promptScene: PROMPT_SCENES.includes(parsed.promptScene) ? parsed.promptScene : "general",
      // v5.0.0：精简工作流（同步版，与异步版 readSentenceState 同一严格布尔校验——两版口径必须一致，
      // 否则同一次会话里"注入提示词读到的值"与"UI/tool 读到的值"会互相矛盾）
      leanWorkflow: typeof parsed.leanWorkflow === "boolean" ? parsed.leanWorkflow : false,
      tools: parsed.tools !== null && typeof parsed.tools === "object" ? parsed.tools : {},
      features: parsed.features !== null && typeof parsed.features === "object" ? parsed.features : {},
  ...
  } catch { return { leanWorkflow: false }; } // v5.0.0：解析失败同样回退默认（理由同上）
}
...
export async function atomicWriteJson(file, obj) {
  ...
  try {
    ...
    // rename 失败（跨设备/目标被占用/只读）回退直写；v4.3.0 修正：直写再失败必须向上抛——
    // 旧版 `catch { /* 忽略 */ }` 让调用方收到"已保存"但磁盘没变（Windows 文件被占用/只读/磁盘满全都静默丢数据）
    try {
      await writeFile(file, JSON.stringify(obj, null, 2), "utf8");
    } catch (writeErr) {
      try { await rm(tmp, { force: true }); } catch { /* 临时文件清理失败不掩盖主错误 */ }
      const err = new Error(`写盘失败（save failed）：${file} —— rename 与直写都未成功：` +
        `rename=${renameErr?.code ?? renameErr?.message ?? renameErr}；write=${writeErr?.code ?? writeErr?.message ?? writeErr}`);
      err.code = "ENOVELWRITEFAIL"; // 调用方（路由层）据此区分"写盘失败 500"与"参数错误 400"
      err.cause = writeErr;
      throw err;
    }
    try { await rm(tmp, { force: true }); } catch { /* 已直写成功，临时文件残留不影响数据 */ }
  }
  });
...
export async function writeSentenceState(patch) {
  const run = stateWriteChain.then(async function () {
    ...
    // v5.0.0：提示词场景（非法值忽略；general 是显式合法值，用于改回通用档）
    ...(PROMPT_SCENES.includes(patch?.promptScene) ? { promptScene: patch.promptScene } : {}),
    // v5.0.0：精简工作流（只接受布尔值；非布尔值静默忽略——与上面 promptScene 处理非法枚举值的既有做法一致，
    // 开关类字段不做隐式转换：字符串 "true"/数字 1 一律当作没传，不写盘、不报错）
    ...(typeof patch?.leanWorkflow === "boolean" ? { leanWorkflow: patch.leanWorkflow } : {}),
  ...
  // v4.3.0 修正：exists 是读取侧由"文件是否存在"推导的输出字段（readSentenceState），不是用户设置——
  // 旧版把 current 里的 exists 一起落盘，且首写时捕获的是"写前"快照 false 并被永久固化在 state.json 里
  // （磁盘上出现 exists:false 却在读侧被覆写成 true，两份口径互相矛盾）。落盘前剔除派生键。
  const { exists: _derivedExists, ...persisted } = next;
  await atomicWriteJson(file, persisted);
  // 返回值保持"读取侧语义"：写完文件必然存在，故 exists 恒为 true（与随后的 readSentenceState() 一致，
  // 也让 index.js 的 effectiveSentenceAnalysis(config, current)/source 判定不再把刚保存的状态当成"默认值"）
...
export function makeStateRoutes(allowLan, config) {
  ...
  const handler = async (req, res) => {
    ...
    if (method === "GET") {
      ...
      return writeJson(res, 200, {
        ...
        // v5.0.0：提示词场景（侧边栏首页开关读它）
        promptScene: state.promptScene ?? "general",
        // v5.0.0：精简工作流（侧边栏首页开关读它；老宿主/老状态文件没有该键 → false，UI 显示关闭态）
        leanWorkflow: state.leanWorkflow ?? false,
        tools: state.tools ?? {},
    ...
    if (method === "POST") {
      ...
      try {
        ...
        // v4.3.0：区分"写盘失败"与"参数错误"——旧版一律 400，磁盘没写成功时 GUI 只看到"参数错误"，误导排查方向
        const status = error?.code === "ENOVELWRITEFAIL" ? 500 : 400;
        return writeJson(res, status, { error: error instanceof Error ? error.message : String(error) });
      }
    }
  ...
  const updateCheckHandler = async (req, res) => {
    ...
    // v4.3.0：checkForUpdate 的稳定形状（ok/stale/lastSuccess/checkedAt）原样透传——本路由是普通 JSON 路由，
    // 无 schema 约束，客户端据此区分"已是最新"与"检查失败"；不在这里补默认值，保持单一事实来源
...
export function upsertCreationSection(text, section, marker) {
  // 段边界 = 下一个标题（标题前可有空行/缩进）/ 下一个【…】段 / 文件尾（防吞掉段后手写内容）
  // v4.3.0 修正：旧前瞻 (?=\n# |\n\n|\s*$) 把「空行」当成段尾，段内一旦有空行就只覆盖到第一个空行——
  // remove 残留（"- 额外：B" 留在文件里）、replace 新旧两段并存（标记出现两次）。
  // 改为只认真正的段尾标记（# 标题 / 【…】段 / 文件尾）。`\r?\n\s*` 前缀是必需的：下一个标题前常有空行，
  // 若按字面写成 (?=\n# ) 则 "\n\n# 主要人物设定" 匹配不上，惰性量词会一路吞掉手写内容；
  // 同时容忍 CRLF（用户用记事本编辑过创作资料）。marker 转义：避免未来引入带正则元字符的标记时炸开。
  ...
  if (has) {
    // v4.0.0 修正：改用函数式替换，避免用户文本里的 "$1"/"$&" 等美元替换模式被展开
  ...
  return {
    ...
    output: {
        schema: {
          ...
          // v4.3.0 修正：删掉从不产出的 files（七个 action 没有一个返回它），补上 read 真实返回的 file
          properties: { book: { type: "string" }, action: { type: "string" }, message: { type: "string" }, content: { type: "string" }, file: { type: "string" } }
        },
        render: (_args, value) => {
    ...
    async execute(args, exec) {
      ...
      if (action === "read") {
        ...
        if (key === "status" || key === "outline") {
          try {
            ...
            // v4.3.0 修正：只对**磁盘上确实存在**的章节报"未回填钩子"。
            // 旧版拿"大纲里出现过的章号"直接比对钩子记录，而大纲按设计会预先列出下一批章节方向
            // （如已写 1-5 章、大纲已有 `- 6 第6章方向…`），于是把尚未动笔的第 6 章也算成"写完却没回填"，
            // 提示与事实不符（实测：雨夜灯只有 5 章，却提示"未回填钩子章节：6"）。
            // 扫描失败时退回旧口径（宁多提醒，不漏提醒）。
            let existing = null;
            try {
              const scanned = await scanChapters(bookDir(root, book));
              existing = new Set(scanned
                .map((c) => c.number)
                .filter((n) => typeof n === "number"));
            } catch { /* 章节扫描失败：退回旧口径（宁多提醒，不漏提醒） */ }
            const missing = outlined.filter((n) => !hooked.includes(n) && (existing === null || existing.has(n)));
      ...
      if (action === "hook") {
        ...
        // v4.3.0 修正：删掉二次 replace(/\n+/g," ")——body 在上方已由 .replace(/\s*\n+\s*/g," ") 单行化，
        // 此处永无换行可替（死代码）；保留的 ^- \d+ 前缀剥离是承重的（调用方误传整行时会写成 "- 1 - 1 …"）
        const oneLineBody = String(body ?? "").replace(/^- \d+ /, "");
        const re = new RegExp("^- " + number + " [^\n]*(?:\n|$)", "m");
        const line = "- " + number + " " + oneLineBody + "\n";
        // v4.0.0 修正：改用函数式替换，避免钩子文本里的 "$&"/"$1" 等美元替换模式把旧行复制进新行
...
export const MIN_BASELINE_CHAPTERS = 2;
export const SMALL_BASELINE_CHAPTERS = 3;
export const SMALL_BASELINE_TOL_PCT = 25;

/**
 * v5.0.0：六维容差带构造——**全插件唯一实现**（novel_fix_plan 与 novel_chapter_brief 共用）。
 *
 * 为什么必须共用：容差决定"多大偏差算出带"。若两条路径用不同公式，同一章会出现
 * 「改稿台说有问题、开写包说没问题」这种互相矛盾的结论，比单条结论错更伤信任。
 * 口径：以**排除本章后的基线**自身的推荐容差 recTol（作者自身章节波动的 1.5σ）为主；
 * 小样本（< 3 章）抬高下限；用户自定义容差（opts.tolerance 优先，其次侧边栏保存的 styleTolerance）只覆盖对应维度。
 */
export function buildTolerance(baseline, sampleCount, opts, stateTolerance) {
  const tol = {};
  if (!baseline) return tol;
  for (const key of METRIC_ORDER) {
    let rt = typeof baseline[key]?.recTol === "number" && baseline[key].recTol > 0 ? baseline[key].recTol : 15;
    if (sampleCount < SMALL_BASELINE_CHAPTERS) rt = Math.max(rt, SMALL_BASELINE_TOL_PCT);
    tol[key] = { low: -rt, high: rt };
  }
  // 用户自定义容差：opts.tolerance 优先，其次侧边栏「风格基线」保存的 styleTolerance（只覆盖对应维度）
  const custom = (opts && opts.tolerance) || stateTolerance;
  if (custom && typeof custom === "object") {
    for (const key of METRIC_ORDER) {
      const t = custom[key];
      if (t && typeof t.low === "number" && typeof t.high === "number") tol[key] = { low: t.low, high: t.high };
    }
  }
  return tol;
}
...
export async function readBookAllText(root, book, exec, opts) {
  ...
  for (const chapter of chapters) {
    ...
    // v4.3.0 修正：逐章拼接补分隔符——旧版 text += t 在"章末无换行 + 下章开头无换行"时把两章粘成一句
    // （跨章句子粘连 → 统计出伪句/伪段落，内容指纹无谓失效）；口径与 buildStyleAnchorPackage 一致（章间必有 \n）
    text += t.endsWith("\n") ? t : t + "\n";
...
export async function buildStyleAnchorPackage(root, book, chapters, exec, preloadedText) {
  // v4.3.0 修正：预读文本是必需契约（root/book/chapters 形参保留仅为兼容 index.js 的位置调用，函数内已不用）。
  // 选择"删除磁盘兜底 + assert"而不是"补测试覆盖"：唯一可达的兜底场景是 preloadedText === ""（整本书都是
  // 空章节文件），那时重读磁盘得到的同样是空文本——兜底本身不产生任何锚段，纯粹是一次无意义的全书双读；
  // 而它把"新调用点漏传预读"这种契约错误掩盖成"空锚包"（静默降级）。assert 必须放在 try 之外：
  // 放进去会被函数尾部的 catch 吞掉，又退化成静默失败。三个调用点（index.js style_check/style_report/new_chapter）
  // 均已显式传入，见 test 侧探针 fixB-13 的源码级核对。
  assert(typeof preloadedText === "string" || Array.isArray(preloadedText),
    "buildStyleAnchorPackage 契约：必须传入已读文本（string 全文 或 {file,text}[]），不再提供去磁盘兜底读取");
  const anchors = [];
  const skeletons = [];
  try {
    // v3.8.0 性能：复用调用方已读文本（preloadedText=string 全文 / {file,text}[] 数组）——大书不再双读
    const allText = typeof preloadedText === "string"
      ? preloadedText
      : preloadedText.map(function (x) { return x.text || ""; }).join("\n");
...
export async function detectChapterBridge(root, book, prevFile, currFile, exec) {
  const out = [];
  // v4.3.0 修正：入口统一净化书名——本函数内原本三处用原始 book 拼路径（章节读取两处 + readSettings），
  // 而同类入口（bookDir/creationDir/readBookAllText）都已净化：书名带非法路径字符时，
  // 前几项检查会读到不存在的路径并被外层 catch 吞成"0 候选"，看起来像"衔接正常"。
  const safeBook = sanitizeSegment(book, "book");
  try {
    const prevText = await readTextFile(join(root, "novels", safeBook, prevFile), exec);
    const currText = await readTextFile(join(root, "novels", safeBook, currFile), exec);
    ...
    try {
      const st = await readSettings(root, safeBook);
    ...
    try {
      // v4.3.0 修正：改用 CREATION_DIR/CREATION_FILES 常量（原为手写 join(root,"novels","创作资料",book,"钩子记录.md")，
      // 与 creationDir() 是同一路径的两份实现，改名/改结构时必然漏改一处）
      const hooksPath = join(creationDir(root, safeBook), CREATION_FILES.hooks);
    ...
    // v4.3.0 修正：读取失败不再静默返回空数组——"文件读不到"与"衔接没有问题"在输出上原本无法区分
    // （整类检查被吞掉，用户会以为检查通过）。透出一条同构候选（带 type/chapters，render 可渲染）。
    out.push({
      type: "衔接·检查跳过",
      detail: "无法读取章节文本（" + safeBook + "：" + prevFile + " → " + currFile + "）：" + String(e?.message ?? e).slice(0, 120) + "——时间/人物/钩子类衔接检查本次全部跳过",
      chapters: [prevFile, currFile]
    });
  }
  return out;
}
...
export function formatChapters(value) {
  ...
  // v4.3.1：同章号多文件提示。这类书按章号读取会被 findChapter 拒绝（避免静默读错稿），
  // 但列表里两条只是"同名条目"，不主动说明的话用户不知道磁盘上是两份文件——必须在这里就看见。
  const dupByNumber = new Map();
  for (const chapter of value.chapters) {
    if (chapter.number === void 0) continue;
    const list = dupByNumber.get(chapter.number) ?? [];
    list.push(chapter.file);
    dupByNumber.set(chapter.number, list);
  }
  const conflicts = [...dupByNumber.entries()].filter(([, files]) => files.length > 1);
  if (conflicts.length > 0) {
    lines.push("", `⚠️ 同章号多文件 ${conflicts.length} 组（按章号读取会因歧义被拒绝；请改名到空闲章号，或用 novel_import 重新导入自动避让）：`);
    for (const [number, files] of conflicts) {
      lines.push(`  第${String(number).padStart(2, "0")}章: ${files.join(" / ")}`);
    }
  }
  lines.push("", "</content>");
  return lines.join("\n");
