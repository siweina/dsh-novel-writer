...
var css = `
  ...
  @keyframes nwflashErr{0%{background:rgba(239,68,68,.18)}100%{background:transparent}}
  ...
  var NW_METRICS = [
        ...
        body = el("div", null,
              ...
              return el("div", { className: "nwTolRow", key: mk2, style: { display: "flex", alignItems: "center", gap: "10px", padding: "9px 2px" } },
                ...
                el("div", { className: "nwTolField", style: { width: "74px", flexShrink: "0", display: "flex", flexDirection: "column", gap: "3px" } },
                  el("div", { style: { display: "flex", alignItems: "center", gap: "4px" } },
                    ...
                    el("input", { type: "number", className: "nwTolInput", placeholder: t("tol.recommend"), value: d.low, min: 0, max: 99, disabled: state.loading, style: { width: "100%", height: "30px", boxSizing: "border-box", border: "1px solid #d4d4d8", borderRadius: "8px", fontSize: "13px", textAlign: "center", background: "#fafafa", color: "#18181b", outline: "none", MozAppearance: "textfield" }, onChange: function (ev) { setDraft(mk2, "low", ev.target.value); } })
                  ),
                ...
                el("div", { className: "nwTolField", style: { width: "74px", flexShrink: "0", display: "flex", flexDirection: "column", gap: "3px" } },
                  el("div", { style: { display: "flex", alignItems: "center", gap: "4px" } },
                    ...
                    el("input", { type: "number", className: "nwTolInput", placeholder: t("tol.recommend"), value: d.high, min: 0, max: 99, disabled: state.loading, style: { width: "100%", height: "30px", boxSizing: "border-box", border: "1px solid #d4d4d8", borderRadius: "8px", fontSize: "13px", textAlign: "center", background: "#fafafa", color: "#18181b", outline: "none", MozAppearance: "textfield" }, onChange: function (ev) { setDraft(mk2, "high", ev.target.value); } })
                  ),
          ...
          el("div", { className: "nwTolBtns", style: { display: "flex", gap: "12px", marginTop: "16px" } },
            el("button", { type: "button", className: "nwBtn nwBtnPrimary" + (state.baselineSaved ? " nwBtnDone" : ""), disabled: state.loading, onClick: saveTol, style: state.baselineSaved ? { background: "linear-gradient(135deg,#10b981,#34d399)", border: "none", color: "#fff", fontWeight: "600", fontSize: "13px", padding: "9px 20px", borderRadius: "10px", cursor: "pointer" } : { background: "linear-gradient(135deg,#6366f1,#8b5cf6)", border: "none", color: "#fff", fontWeight: "600", fontSize: "13px", padding: "9px 20px", borderRadius: "10px", cursor: "pointer" } }, state.baselineSaved ? "✓ " + t("baseline.saved") : "💾 " + t("baseline.save")),
            el("button", { type: "button", className: "nwBtn nwBtnGhost", disabled: state.loading, onClick: resetTol, style: { background: "transparent", border: "1px solid #cbd5e1", color: "#475569", fontSize: "13px", padding: "9px 18px", borderRadius: "10px", cursor: "pointer" } }, "↺ " + t("baseline.reset"))
          )
        );
        ...
        body = el("div", null,
          ...
          // 场景只在完整档下有意义（精简档只说明"有这套工具"），因此档位非 full 时整行置灰。
          ...
          el("div", { className: "nwRow " + (state.systemPromptMode === "full" ? "nwRowOn" : "nwRowOff") },
            el("div", { className: "nwRowText" },
              ...
              el("div", { className: "nwRowHint" }, t("panel.spSceneHint"))
            ),
            el("div", { className: "nwSeg" },
              ["general", "writing", "revising", "auditing", "setup"].map(function (s) {
                var cur = state.promptScene || "general";
                return el("button", { key: s, type: "button", className: "nwSegBtn" + (cur === s ? " nwSegBtnOn" : ""), disabled: state.loading || state.systemPromptMode !== "full",
                  onClick: function () { if (cur !== s) props.toggle({ promptScene: s }); } }, t("sc." + s));
              })
            )
          ),
          // v5.0.0：精简工作流——全局开关，紧挨「提示词档位 / 提示词场景」两行之下、与相邻开关同一套 switch/badge 组件。
          // 与上一条「提示词场景」的区别：它不受"仅完整档生效"限制（不给 disabled 逻辑），任何时候都可切换；
          ...
          el("div", { className: "nwRow " + (state.leanWorkflow ? "nwRowOn" : "nwRowOff") },
            ...
            el("div", { className: "nwSwitchWrap" },
              el(Switch, { checked: state.leanWorkflow === true, disabled: state.loading, onChange: function (value) { props.toggle({ leanWorkflow: value }); } }),
