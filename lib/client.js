/**
 * dsh-novel-writer — 浏览器端（client half, v3.9.5）
 *
...
"group.allOn": "All on",
      ...
      var api = {
        ...
        rawToggle: function (wantOn) {
          ...
          rawTimer = setInterval(function () {
            ...
            if (n <= 0) { clearInterval(rawTimer); rawTimer = null; api.set({ rawCountdown: 0 }); }
            else api.set({ rawCountdown: n });
          }, 1000);
        },
        rawConfirm: function () { if (rawTimer) { clearInterval(rawTimer); rawTimer = null; } api.set({ rawModal: false, rawPromiseOpen: true, rawPromiseText: "", rawPromiseOk: false }); },
        rawCancel: function () { if (rawTimer) { clearInterval(rawTimer); rawTimer = null; } api.set({ rawModal: false, rawCountdown: 0, rawPromiseOpen: false, rawPromiseText: "", rawPromiseOk: false }); },
    ...
    function mountSidebarEntry(controller, onOpen) {
      ...
      try { waitObserver.observe(document.body, { childList: true, subtree: true }); } catch (e) { /* body 不可观察时静默降级 */ }
      ...
      var entry = function (title, hint, count, viewName) {
        return el("button", { type: "button", className: "nwNavEntry", onClick: function () { props.controller.openView(viewName); } },
          ...
          el("div", { className: "nwNavEntryRight" },
            count === null ? null : el("span", { className: "nwBadge " + (count > 0 ? "nwBadgeOn" : "nwBadgeOff") }, (t("group.onCount") || "{n} 开").replace("{n}", count)),
...
;
  ...
  var NW_METRICS = [
        ...
        var openReport = function (file) {
          ...
          }).catch(function () { if (reqId !== reportReqId) return; props.controller.set({ reportContent: t("reports.readFailDetail") }); });
        };
        ...
        body = el("div", null,
          ...
          el("button", { key: "reportsEntry", type: "button", style: { display: "flex", alignItems: "center", width: "100%", marginTop: "8px", padding: "9px 12px", borderRadius: "10px", border: "1px solid #e2e8f0", background: "#fff", cursor: "pointer", textAlign: "left" }, onClick: function () {
            props.controller.set({ reportsGroups: null, reportContent: null });
            fetch("/api/dsh-novel-writer/reports", { headers: { accept: "application/json" } }).then(function (r2) { if (!r2.ok) throw new Error("HTTP " + r2.status); return r2.json(); }).then(function (d) {
              props.controller.set({ reportsGroups: d.groups || [] });
            }).catch(function () { props.controller.set({ reportsGroups: [] }); });
    ...
    function mountPanel(controller, toggle) {
      ...
      var ensure = function () {
        ...
        if (root !== void 0) { try { root.unmount(); } catch (error) { /* ignore */ } root = void 0; }
        if (container !== void 0) { try { container.remove(); } catch (error) { /* ignore */ } container = void 0; }
