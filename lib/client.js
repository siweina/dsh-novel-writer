...
var css = `
  ...
  .nwSeg{display:flex;gap:4px;flex:none}
  .nwSegBtn{appearance:none;border:1px solid #cbd5e1;background:#fff;color:#475569;font-size:12px;padding:5px 10px;border-radius:8px;cursor:pointer;transition:background .16s,border-color .16s,color .16s}
  .nwSegBtn:hover{border-color:#94a3b8}
  .nwSegBtnOn{background:#22c55e;border-color:#22c55e;color:#fff;font-weight:600}
  .nwSegBtn:disabled{opacity:.6;cursor:default}
...
"group.allOn": "All on",
    ...
    // ---- 工具清单（与宿主端 ALL_TOOLS 一致；v4.0.0：仅作宿主清单对照，开关计数已改用 TOOL_GROUPS，见主面板 onTools）----
      ...
      var api = {
        ...
        refresh: function () {
          ...
          return fetchState().then(function (remote) {
            api.set({
              ...
              styleTolerance: (api.getSnapshot().rev === revAt ? (remote.styleTolerance || null) : api.getSnapshot().styleTolerance || null),
              creationProfile: (api.getSnapshot().rev === revAt ? (remote.creationProfile || null) : api.getSnapshot().creationProfile || null),
              creationProfiles: (api.getSnapshot().rev === revAt ? (remote.creationProfiles || {}) : api.getSnapshot().creationProfiles || {}),
              books: remote.books || [],
            ...
            api.set({ hostOk: false, loading: false, refreshing: false, revealMsg: t("reports.readFail") + "：" + t("state.unreachable"), revealErr: true }, { silent: true });
            return null;
          });
        },
        openDir: function (target) {
          ...
          fetch("/api/dsh-novel-writer/reveal", {
            ...
            api.set({ revealMsg: t("plot.revealErr") + "：" + t("state.unreachable"), revealErr: true }, { silent: true });
          }).finally(function () {
    ...
    function mountSidebarEntry(controller, onOpen) {
      ...
      var langObserver = new MutationObserver(function () { applyLabel(); });
...
;
  ...
  var NW_METRICS = [
              ...
              return el("div", { className: "nwTolRow", key: mk2, style: { display: "flex", alignItems: "center", gap: "10px", padding: "9px 2px" } },
                ...
                el("div", { className: "nwTolField", style: { width: "74px", flexShrink: "0", display: "flex", flexDirection: "column", gap: "3px" } },
                  el("div", { style: { display: "flex", alignItems: "center", gap: "4px" } },
                    ...
                    el("input", { type: "number", className: "nwTolInput", placeholder: t("tol.recommend"), value: d.low, min: 0, max: 99, style: { width: "100%", height: "30px", boxSizing: "border-box", border: "1px solid #d4d4d8", borderRadius: "8px", fontSize: "13px", textAlign: "center", background: "#fafafa", color: "#18181b", outline: "none", MozAppearance: "textfield" }, onChange: function (ev) { setDraft(mk2, "low", ev.target.value); } })
                  ),
                ...
                el("div", { className: "nwTolField", style: { width: "74px", flexShrink: "0", display: "flex", flexDirection: "column", gap: "3px" } },
                  el("div", { style: { display: "flex", alignItems: "center", gap: "4px" } },
                    ...
                    el("input", { type: "number", className: "nwTolInput", placeholder: t("tol.recommend"), value: d.high, min: 0, max: 99, style: { width: "100%", height: "30px", boxSizing: "border-box", border: "1px solid #d4d4d8", borderRadius: "8px", fontSize: "13px", textAlign: "center", background: "#fafafa", color: "#18181b", outline: "none", MozAppearance: "textfield" }, onChange: function (ev) { setDraft(mk2, "high", ev.target.value); } })
                  ),
        ...
        var openReport = function (file) {
          ...
          }).catch(function () { if (reqId !== reportReqId) return; props.controller.set({ reportContent: t("reports.readFailDetail") }, { silent: true }); });
        };
        ...
        if (view === "creation") {
          ...
          body = el("div", null,
            ...
            el("div", { style: { display: "flex", gap: "8px", marginTop: "8px", marginBottom: "10px" } },
              el("input", { id: "nwNewBookInput", type: "text", value: newBookName, placeholder: t("creation.newPlaceholder"), style: { flex: "1", height: "34px", boxSizing: "border-box", padding: "0 12px", border: "1px solid #d4d4d8", borderRadius: "10px", fontSize: "13px", background: "#fff", color: "#18181b", outline: "none", transition: "border-color .15s ease, box-shadow .15s ease" }, onInput: function (ev) { props.controller.set({ creationNewBook: ev.target.value }, { silent: true }); }, onFocus: function (ev) { ev.target.style.borderColor = "#6366f1"; ev.target.style.boxShadow = "0 0 0 3px rgba(99,102,241,.14)"; }, onBlur: function (ev) { ev.target.style.borderColor = "#d4d4d8"; ev.target.style.boxShadow = "none"; }, onKeyDown: function (ev) { if (ev.key === "Enter") { ev.preventDefault(); createNewBook(); } } }),
        ...
        // v4.0.0 修正：计数与工具页渲染同源（TOOL_GROUPS 共 15 项）；ALL_TOOLS 含 novel_sentence_config，
        // 该项没有 UI 行（宿主写路径会剥离该键），用 ALL_TOOLS 计数会出现"16 开"却只有 15 行
        ...
        body = el("div", null,
          ...
          (state.saveFailed || state.hostOk === false) ? el("div", { key: "saveBanner", style: { padding: "7px 10px", marginBottom: "8px", borderRadius: "8px", background: "#fef3c7", border: "1px solid #fcd34d", color: "#92400e", fontSize: "11px", lineHeight: "1.5" } }, state.hostOk === false ? t("panel.localOnly") : t("panel.saveFailed")) : null,
          ...
          el("div", { className: "nwRow " + (state.systemPromptMode !== "off" ? "nwRowOn" : "nwRowOff") },
            ...
            el("div", { className: "nwSeg" },
              ["off", "brief", "full"].map(function (m) {
                return el("button", { key: m, type: "button", className: "nwSegBtn" + (state.systemPromptMode === m ? " nwSegBtnOn" : ""), disabled: state.loading,
                  onClick: function () { if (state.systemPromptMode !== m) props.toggle({ systemPromptMode: m }); } }, t("sp." + m));
              })
            )
          ),
          // 非净化模式（主页直接可见，红色危险选项）
          ...
          el("div", { key: "statsCard", style: { marginTop: "10px", padding: "10px 12px", borderRadius: "10px", border: "1px solid #e2e8f0", background: "#fff" } },
            ...
            el("div", { style: { display: "flex", flexDirection: "column", gap: "5px" } },
              state.booksStats === null
                ? el("div", { style: { fontSize: "11px", color: "#f59e0b" } }, t("stats.pending"))
                ...
                : (state.booksStats || []).slice(0, 6).map(function (bs) {
                    ...
                    return el("div", { key: bs.name, style: { display: "flex", alignItems: "center", gap: "8px", fontSize: "11px" } },
                      ...
                      bs.recent7Chars > 0
                        ? el("span", { style: { color: "#10b981", fontWeight: "600" } }, "🔥 " + fmtChars(bs.recent7Chars) + " " + t("stats.week"))
          ...
          el("button", { key: "reportsEntry", type: "button", style: { display: "flex", alignItems: "center", width: "100%", marginTop: "8px", padding: "9px 12px", borderRadius: "10px", border: "1px solid #e2e8f0", background: "#fff", cursor: "pointer", textAlign: "left" }, onClick: function () {
            ...
            fetch("/api/dsh-novel-writer/reports", { headers: { accept: "application/json" } }).then(function (r2) { if (!r2.ok) throw new Error("HTTP " + r2.status); return r2.json(); }).then(function (d) {
              props.controller.set({ reportsGroups: d.groups || [] }, { silent: true });
            }).catch(function () {
              // v4.0.0 修正：失败不再伪装成"暂无报告"（空数组），改为错误提示条，避免用户去跑分析而不是重试
              props.controller.set({ reportsGroups: [], revealMsg: t("reports.readFail") + "：" + t("reports.readFailDetail"), revealErr: true }, { silent: true });
            });
          ...
          el("div", { key: "demoCard", style: { marginTop: "10px", padding: "10px 12px", borderRadius: "10px", border: "1px dashed #c7d2fe", background: "#f8f9ff" } },
            el("button", { type: "button", disabled: state.demoLoading, style: { display: "flex", alignItems: "center", gap: "8px", width: "100%", background: "transparent", border: "none", padding: 0, cursor: state.demoLoading ? "default" : "pointer", textAlign: "left" }, onClick: function () {
              ...
              fetch("/api/dsh-novel-writer/demo", { headers: { accept: "application/json" } }).then(function (r2) { if (!r2.ok) throw new Error("HTTP " + r2.status); return r2.json(); }).then(function (d) {
                props.controller.set({ demoReport: d.ok ? d.report : (t("demo.fail") + "：" + (d.error || "")), demoLoading: false }, { silent: true });
              }).catch(function () { props.controller.set({ demoReport: t("demo.fail") + "：" + t("reports.readFailDetail"), demoLoading: false }, { silent: true }); });
            } },
    ...
    function apply(ctx) {
      ...
      var load = async function () {
        ...
        try {
          ...
          var revSame = controller.getSnapshot().rev === revAt;
      ...
      var toggle = async function (patch) {
        ...
        controller.set({ hostOk: false, saveFailed: true });
        try {
          var snapshot = controller.getSnapshot();
          // v3.5.0 #38：宿主不可达时完整持久化（刷新不丢 tools/features/容差/设定）
          localStorage.setItem(STORAGE_KEY, JSON.stringify({ enabled: snapshot.enabled, autoAnalyze: snapshot.autoAnalyze, tools: snapshot.tools || {}, features: snapshot.features || {}, styleTolerance: snapshot.styleTolerance || null, creationProfile: snapshot.creationProfile || null, creationProfiles: snapshot.creationProfiles || {} }));
        } catch { /* ignore */ }
      };
