...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var zh = {
      ...
      "feature.semanticEmbedding.desc": "本地 bge-small-zh 模型：语义检索伏笔/情感段落 + 风格语义对比。0 token，默认自动启用，模型缺失时自动回退纯规则。",
      "feature.semanticSearch": "语义检索",
      "feature.semanticSearch.desc": "novel_semantic_search：自然语言搜全书语义相关段落（伏笔/情感场景）。",
      "feature.semanticStyle": "语义风格对比",
      "feature.semanticStyle.desc": "novel_style_check 附加语义相似度维度（规则指纹之外）。",
    ...
    var ALL_TOOLS = ["novel_books", "novel_chapters", "novel_read", "novel_keywords", "novel_new_chapter", "novel_import", "novel_sentence_analysis", "novel_sentence_config", "novel_style_check", "novel_plot", "novel_settings", "novel_summary", "novel_continuity_check", "novel_semantic_search"];
    ...
    function PanelView(props) {
      ...
      var featureRows = ["emotionCaveat", "genreTheme", "emotionComplexity", "semanticEmbedding"].map(function (name) {
        var fOn = f[name] !== false;
        var extra = null;
        if (name === "semanticEmbedding" && fOn) {
        ...
        if (name === "rawWriting") {
          // 非净化模式：红色标注 + 特殊开关流程（开需双重确认）
          return el("div", { className: "nwToolRow" + (fOn ? " nwToolRowOn" : " nwToolRowRaw"), key: name },
            el("div", { className: "nwToolLabel" },
              el("div", { className: "nwToolLabelRaw" }, "🔞 " + t("feature.rawWriting")),
              el("div", { className: "nwToolDesc nwRawDanger" }, t("raw.danger")),
              fOn ? el("div", { className: "nwRawOn" }, t("raw.on")) : null
            ),
            el("button", { type: "button", role: "switch", "aria-checked": fOn ? "true" : "false", className: "nwSwitchSmall" + (fOn ? " nwSwitchSmallOn" : ""), disabled: state.loading,
              onClick: function () { props.controller.rawToggle(!fOn); } },
              el("span", { className: "nwSwitchSmallKnob" })
            )
          );
        }
        return switchRow(name, fOn, function (n, v) { props.toggle({ features: (function (patch) { patch[n] = v; return patch; })({}) }); }, extra);
      });
      var toolRows = ALL_TOOLS.map(function (name) {
      ...
      if (view === "features") {
        ...
        var onFeatures = ["emotionCaveat", "genreTheme", "emotionComplexity", "semanticEmbedding"].filter(function (k) { return f[k] !== false; }).length;
