...
function modelPresent() {
  ...
  return (
    ...
    (fs.existsSync(path.join(MODEL_DIR, "tokenizer.json")) || fs.existsSync(path.join(MODEL_DIR, "tokenizer.json.gz"))) &&
...
function initEngine() {
  ...
  enginePromise = (async () => {
    try {
      ...
      // v4.0.1：tokenizer.json 以 gzip 随包分发（原文件 439KB 超出商店单文件上限），加载时解压
      const tjPath = path.join(MODEL_DIR, "tokenizer.json");
      const tj = JSON.parse(fs.existsSync(tjPath) ? fs.readFileSync(tjPath, "utf8") : gunzipSync(fs.readFileSync(tjPath + ".gz")).toString("utf8"));
 ...
 * v4.3.0 修复（E9）：字段加长度前缀定界——旧版只用 \u0000/\u0001 分隔，字段自身含分隔符时边界有歧义，
 * fingerprint([{id:"a\u0000b",text:"c"}]) 与 fingerprint([{id:"a",text:"b\u0000c"}]) 得到同一指纹（实测 true）：
 * 分块集合不同却当成"内容未变"，脏命中会复用旧索引/旧 styleproto 段落向量。
 * 长度前缀（UTF-16 码元数）让边界无歧义，分隔符保留仍可读。算法一变，既存索引/风格缓存指纹一次性失效重建，属预期的一次性代价。
 * 注：saveIndex 用同一函数对实际写入条目重算指纹，两侧编码口径逐字节一致（完整索引必然相等）。 */
function fingerprint(chunks) {
  const h = createHash("sha1");
  for (const c of chunks || []) {
    const id = String(c.id);
    const text = String(c.text);
    h.update(id.length + ":" + id);
    h.update("\u0000");
    h.update(text.length + ":" + text);
    h.update("\u0001");
  }
  return h.digest("hex");
}
 ...
 * v4.3.0 注释修正（E10c）：函数签名为 (root, book, index) 三个形参——旧 JSDoc 声称"第四参数 fp 已废弃、调用方多传的实参会被忽略"，
 * 但全项目三处调用（index.js 两处 + core.js 一处）都只传 3 个实参，从未传过 fp（不写行号：并行改动会漂移）。
 * 内容指纹确实不由调用方传入，而是下方按实际写入条目内部重算（见 actualFp）。 */
...
function chunkText(text) {
  ...
  // v4.3.0 修正（P1-4）：一并归一 U+2028/U+2029/U+0085——它们同样是行分隔符，旧版既会让
  // 下方 /.{1,150}/g 对"整行只有 U+2028"的超长行返回 null（for...of null 抛 TypeError，
  // 使整书语义检索/隐性情感/风格对比同时降级），又会被静默丢弃导致 chunk 与原文不再逐字一致。
  ...
  for (const line of lines) {
    ...
    // v4.3.0 修正（P1-4）：`.` 不匹配 U+2028/U+2029，match 会返回 null → 改为 [\s\S] 并保留兜底，
    // 任何输入都不再让切块抛错（上面的归一化已把它们转成 \n，这里是第二道保险）。
    const pieces = line.length > 150 ? (line.match(/[\s\S]{1,150}/g) || [line]) : [line];
...
// vibe.js 用 IMPLICIT_EMOTION_PROTOTYPES（v4.3.0 增补 MODEL_NAME：styleproto 缓存 payload 需写入模型标识，
// 与索引缓存的 {model,dim} 校验口径一致）；index.js 用 status/isAvailable/embed/cosine/search/saveIndex/loadIndexMeta/
// fingerprint/buildIndexIncremental/chunkText/detectImplicitEmotions/engine。embedMany/textHash/cachePath/MODEL_DIR 保留为内部函数。
export { status, isAvailable, embed, cosine, search, saveIndex, loadIndexMeta, fingerprint, buildIndexIncremental, chunkText, detectImplicitEmotions, IMPLICIT_EMOTION_PROTOTYPES, MODEL_NAME, engine };
