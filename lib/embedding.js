...
let engine = { available: false, error: null, tokenizer: null, session: null, ort: null, wasmPaths: null, failedAt: null };
...
async function isAvailable() {
  ...
  // v3.9.1 失败粘性修复：initEngine 缓存了失败的 enginePromise——超过 30s 冷却即作废重试（仍失败则 failedAt 刷新，再等 30s）
  if (!engine.available && engine.failedAt && Date.now() - engine.failedAt > 30 * 1000) {
    enginePromise = null;
    engine.failedAt = null;
  }
  await initEngine();
...
async function embedMany(chunks) {
  ...
  // v3.9.1：第一轮失败（抛错或返回 null）的块，第二轮再 embed 一次；仍失败才保留 null 由 filter 剔除
...
async function search(query, index, k = 5) {
  ...
  // v3.9.1 注释修正：原「首次 built 的索引为 512 维」描述已过期——saveIndex/buildIndexIncremental 现在统一 128 维落库；
  // 此处保留 512/128 双维防御（cosine 要求等长，否则恒返回 0），query 为 512 维模型直出时与 128 维索引匹配需降采样
...
function downsample(vec) {
  ...
  const v = raw.length > 128 ? raw.filter((_, i) => i % 4 === 0) : raw;
...
/** 保存索引（v2.6.0：统一 128 维 + 4 位小数降精度 + zstd 压缩，体积约 1/10；fp=内容指纹）。v3.9.1：正文截断上限 200 字（与检索返回一致）。 */
...
/** 读取索引缓存（不存在返回 null）。v3.9.1：逐向量长度校验——任一 vec 维度与 EMBED_DIM 不符即视为缓存损坏（旧 128 维缓存合法，不被误杀）。 */
function loadIndex(root, book) {
  try {
    ...
    for (const item of data.items) {
      if (!Array.isArray(item?.v) || item.v.length !== EMBED_DIM) return null;
    }
    return data.items.map((item) => ({ id: item.id, text: item.t, vec: item.v, chapter: item.ch ?? null }));
  } catch { return null; }
}

/** 读取索引缓存 + 内容指纹（旧格式无 fp 时 fp=null，视为失效需重建；不靠版本号失效）。v3.9.1：同 loadIndex 的逐向量长度校验。 */
...
function reset() { enginePromise = null; engine = { available: false, error: null, tokenizer: null, session: null, ort: null, wasmPaths: null, failedAt: null }; }
