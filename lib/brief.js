...
export async function buildChapterBrief(root, book, chapterArg, opts, exec) {
  ...
  const checklist = ["先读【anchors 锚段】再动笔，按原著味道写；baseline 数字只做写完后的校验"];
  ...
  if (baseline) checklist.push("写完对照六维基线（baseline 的 μ±σ）与锚段，跑 novel_style_check 自检");
  if (chapter && typeof chapter.number === "number") {
    checklist.push("写完回填钩子：novel_outline action=hook number=" + chapter.number);
  }
