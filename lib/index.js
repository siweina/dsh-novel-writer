...
const FEATURE_DEFAULTS = Object.freeze({ emotionCaveat: true, genreTheme: true });
...
function registerNovelSentenceConfig(ctx, config) {
  ctx.tools.register({
    ...
    description: "查看或修改写作助手功能的开关状态：enabled（总开关）、autoAnalyze（分析作品时是否主动使用句式分析）、tools（各工具独立开关，如 { novel_plot: false }）、features（功能开关：emotionCaveat=情感净化预警 / genreTheme=题材与流派检测，如 { emotionCaveat: false }）。GUI 开关位于 Web 侧边栏「写作助手功能」面板，本工具与其同步。",
    parameters: {
      type: "object",
      properties: {
    ...
    output: {
      schema: {
        ...
        properties: {
          ...
          features: { type: "object", additionalProperties: true, description: "各功能开关状态（emotionCaveat/genreTheme）。" }
        },
        required: ["file", "enabled", "autoAnalyze", "tools", "features"]
      },
      render: (_args, value) => {
    ...
    async execute(args) {
      ...
      if (action === "set") {
        ...
        if (args?.features !== null && typeof args?.features === "object") {
          const featPatch = {};
          for (const name of ["emotionCaveat", "genreTheme"]) {
      ...
      return { file: stateFilePath(), enabled: after.enabled, autoAnalyze: after.autoAnalyze, source, updated, tools, features };
    }
  });
}
