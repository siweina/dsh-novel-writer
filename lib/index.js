...
function normalizeSettingEntry(entry) {
  ...
  for (const key of ["description", "traits", "relationships", "firstSeen", "notes", "owner", "status", "lastSeen", "day", "event", "chapter"]) {
    if (entry[key] !== void 0 && entry[key] !== null) out[key] = String(entry[key]);
  }
...
function registerNovelSettings(ctx, config) {
  ctx.tools.register({
    ...
    parameters: {
      ...
      properties: {
        book: { type: "string", description: "书名。" },
        category: { type: "string", enum: ["character", "location", "item", "timeline"], description: "表类别（list/add/update/delete/scan 需要）。" },
        action: { type: "string", enum: ["list", "add", "update", "delete", "scan"], description: "list=查看（默认）；add=登记；update=修改；delete=删除；scan=扫描章节提取候选。" },
    ...
    output: {
      ...
      render: (_args, value) => {
        ...
        const categoryLabel = { character: "人物卡", location: "地点卡", item: "道具清单", timeline: "时间线" }[value.category] ?? value.category;
        const list = value.characters ?? value.locations ?? value.items ?? value.timeline ?? [];
        lines.push(`${categoryLabel}（${list.length} 条）：`);
        for (const e of list) {
          const extra = [e.traits, e.description, e.relationships, e.owner, e.status, e.lastSeen, e.day + (e.event ? " " + e.event : "")].filter(Boolean).join(" | ");
    ...
    async execute(args, exec) {
      const book = sanitizeSegment(requiredString(args, "book"), "book");
      const category = ["character", "location", "item", "timeline"].includes(args?.category) ? args.category : "character";
      const action = ["add", "update", "delete", "scan"].includes(args?.action) ? args.action : "list";
      const root = resolveRoot(config, args, exec);
      const data = await readSettings(root, book);
      const now = new Date().toISOString();
      let message = "";
      const categoryLabel = { character: "人物卡", location: "地点卡", item: "道具清单", timeline: "时间线" }[category] ?? category;
      const listKey = { character: "characters", location: "locations", item: "items", timeline: "timeline" }[category];
      ...
      if (action === "add") {
        ...
        const entry = { name };
        for (const key of ["description", "traits", "relationships", "firstSeen", "owner", "status", "lastSeen", "event", "chapter", "notes"]) {
        ...
        if (category === "character") result.characters = data.characters.map(normalizeSettingEntry);
        if (category === "location") result.locations = data.locations.map(normalizeSettingEntry);
        if (category === "item") result.items = data.items.map(normalizeSettingEntry);
        if (category === "timeline") result.timeline = data.timeline.map(normalizeSettingEntry);
        return result;
      }
