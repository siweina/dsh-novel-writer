...
function makeStateRoutes(allowLan) {
  ...
  const handler = async (req, res) => {
    ...
    if (method === "GET") {
      ...
      return writeJson(res, 200, { enabled: state.enabled, autoAnalyze: state.autoAnalyze, file: stateFilePath() });
    }
    if (method === "POST") {
...
function registerStyleConfigRoute(ctx, config) {
  try {
    ctx.inject(["webServer"], (wctx) => {
      const routes = makeStateRoutes(config?.allowLanState === true);
      wctx.effect?.(() => {
...
function registerNovelSentenceAnalysis(ctx, config) {
  ctx.tools.register({
    ...
    async execute(args, exec) {
      ...
      // v0.6.0：分析结果缓存 + 报告导出（用户建议的临时文件方案）
      const cacheDir = join(homedir(), ".dsh", "dsh-novel-writer", "cache");
...
function plotsFile(root, book) {
  return join(root, ".novel-writer", sanitizeSegment(book, "book") + ".json");
}
async function readPlots(root, book) {
  try {
    const parsed = JSON.parse(await readFile(plotsFile(root, book), "utf8"));
...
function registerNovelPlot(ctx, config) {
  ctx.tools.register({
    ...
    parameters: {
      ...
      properties: {
        book: { type: "string", description: "书名。" },
        action: { type: "string", enum: ["list", "add", "update", "done", "delete"], description: "list=查看（默认）；add=登记新伏笔；update=修改；done=标记已回收；delete=删除。" },
    ...
    output: {
      ...
      render: (_args, value) => {
        ...
        for (const e of open) lines.push(`  - [${e.id}] ${e.content}${e.chapter ? "（出自 " + e.chapter + "）" : ""}${e.note ? "｜" + e.note : ""}`);
        if (done.length > 0) {
    ...
    async execute(args, exec) {
      const book = sanitizeSegment(requiredString(args, "book"), "book");
      const action = ["add", "update", "done", "delete"].includes(args?.action) ? args.action : "list";
      const root = resolveRoot(config, args, exec);
      ...
      if (action === "add") {
        ...
        entries.push({ id, content, chapter: optionalString(args, "chapter"), note: optionalString(args, "note"), status: "open", createdAt: now, updatedAt: now });
        message = `已登记伏笔 #${id}`;
      ...
      return { book, action, entries, ...message ? { message } : {} };
    }
  });
}
...
const WORKFLOW_TEXT = [
  ...
  "   开关默认开启；统一在 Web GUI 侧边栏「句式分析」面板调整（设置 > 插件配置 仅显示状态），也可用 novel_sentence_config。动手前可先调用 novel_sentence_config 查看状态：",
  ...
  "8. 工具链提示：续写/分析前可用 novel_plot 查看未回收伏笔，保持剧情连贯；写完新章节可用 novel_style_check 做风格自检（相似度+偏差清单）；",
  "   novel_sentence_analysis 结果自动缓存到 ~/.dsh/dsh-novel-writer/cache/（返回值含 reportFile 字段），重复分析直接命中缓存，需要重算时传 fresh=true。"
].join("\n");
