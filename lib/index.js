/**
 * dsh-novel-writer — 小说写作助手插件（v4.0.0：全量缺陷整修——句式/情感统计口径、缓存与语义索引、浏览器端状态与竞态、测试与发布流程）
...
// v4.0.0 修正：删除从未使用的 writeFileSync / sep / statSync（statSync 随 GET /state 异步化一并移除）
...
// v3.5.0 #22：分析缓存算法版本——升级/改算法后旧缓存失效
const CACHE_VERSION = PLUGIN_VERSION;
...
async function resolveAmbiguousCarriers(implicit) {
...
async function chapterStats(bookPath, chapter, exec) {
...
function tallyCjkRun(run, cjkBigrams, cjkTrigrams) {
...
/** 句式分析默认配置。 */
const SENTENCE_ANALYSIS_DEFAULTS = Object.freeze({ enabled: true, autoAnalyze: true, systemPromptMode: "brief" }); // v4.0.0：系统提示词注入档位 off/brief/full（默认精简）
...
// v4.0.0：词表扫描公共件——
// ① 静态词表排序缓存（旧版每次调用都对同一张表 slice().sort() 重排）
// ② 占用位图单遍扫描（旧版每次命中都重建整段文本，整体代价 O(命中数 × 文本长度)）
const SORTED_MARKER_WORDS = new Map();
function sortedMarkerWords(words) {
...
/** 同表"先长后短"最长匹配计数：已被更长词占用的字符位置不再被短词重复计（语义与旧占位替换一致）。
 *  @returns {{word:string,count:number}[]} 只含命中数 > 0 的词 */
function scanWordHits(text, words) {
 ...
 * v3.9.1 修复：scope=章节文件名时只在章节范围内检测/复用——此前整书检索会把别章命中混入本章报告，
 * 且单章缓存命中时旧版越界结果会被直接复用（早退），须校验 hits 的 chapter 全部落在 scope 内才早退。 */
async function enrichSemanticImplicit(state, root, book, result, exec, scope) {
  try {
...
/** v3.5.0 #18：原子 JSON 写入——临时文件 + rename，防半截文件/并发互相覆盖。 */
// v3.5.0 #18：per-path 写队列——写端串行，防并发互相覆盖
...
// v3.9.5：按文件串行化“读-改-写”事务——并发 add/update/delete 不再基于同一旧快照整表覆盖（丢更新）
const txQueues = new Map();
async function withFileTx(file, fn) {
...
/** v3.9.5：打开系统文件管理器定位到某目录（Windows: explorer /select），回传是否启动成功。 */
...
/** 句式类型标签直接复用 analysis.js 的 CATEGORY_LABELS（v3.9.5：删除本地重复定义，统一“省略留白”叫法）。 */
const EMOTION_CODE = { joy: "喜", anger: "怒", sorrow: "哀", fear: "惧", surprise: "惊", neutral: "中性" };

/** v3.9.5：brief 文案统一生成（缓存命中/未命中两条路径共用，消除“留白较多”尾巴不一致）。 */
function buildBriefLine(result, scopeText) {
...
// v3.9.5：骨架标题白名单提升为模块级常量（原两处循环内重复 new Set、且缩进错乱）
const CREATION_TEMPLATE_TITLES = new Set(["# 创作设定", "# 主要人物设定", "# 次要人物设定", "# 剧情大纲", "# 钩子记录", "# 创作状态卡"]);
...
/** v4.0.0：把语义层结果（隐性情感 semanticImplicit / 意象歧义裁决 implicitResolved）写回全书分析缓存，
 * 连同内容指纹 th（semanticFp）；下次命中直接复用——不再重跑索引+原型推理。读改写只动语义键，
 * 其余字段（detection/genre/theme/未来新增键）原样保留；语义为空时删除键（功能关闭/引擎不可用 → 清除旧残留）。 */
async function writeBookSemanticCache(info, semanticImplicit, implicitResolved) {
...
/** v4.0.0：摘要条目字段白名单清洗（get/add/update/list 共用）。
 *  旧版 get 直接透传磁盘原文，脏数据（多余键/undefined）会被宿主 output schema 校验整次拒绝。 */
function cleanSummaryEntry(s) {
...
async function detectChapterBridge(root, book, prevFile, currFile, exec) {
  const out = [];
  try {
...
/** v3.9.5：cleanOutput 补全——Date→ISO 字符串、Map/Set→数组、RegExp→字符串、NaN→null（宿主 lossless JSON 契约）。 */
