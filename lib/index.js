...
function decodeTextBuffer(buf, filePath) {
  ...
  if (nulRatio > 0.05) {
    // v3.5.0 #1：UTF-16LE 的 NUL 在奇数下标（41 00），BE 在偶数（00 41）——修正判反
      ...
      if (mode === "apply") {
        for (const group of groups) {
          ...
          for (const row of group.files) {
            ...
            // v3.5.0 #19：目标同名已存在 → 自动改名（-2/-3…），不静默覆盖、move 模式不丢源
            let destFile = basename(row.file);
            const destBase = destFile.replace(/\.(md|txt)$/i, "");
            const destExt = destFile.match(/\.(md|txt)$/i)?.[0] || "";
            let k = 2;
            while (existsSync(join(destDir, destFile))) {
              destFile = destBase + "-" + k + destExt;
              k += 1;
            }
...
function openInExplorer(dir) {
  if (process.platform === "win32") {
    ...
    // v3.5.0 #12：仅用 execFile 数组直调（cmd start / explorer.exe），移除 powershell 命令拼接——路径含特殊字符也不会注入
    // v3.5.0 #12：仅 explorer.exe 数组直调（execFile 不经 shell，路径含 & 等特殊字符也无法注入）
    const ways = [
    ...
    try {
      ...
      for (const b4 of books4) {
        ...
        for (const e4 of await readdir(d4, { withFileTypes: true })) {
          ...
          const TEMPLATE_TITLES4 = new Set(["# 创作设定", "# 主要人物设定", "# 次要人物设定", "# 剧情大纲", "# 钩子记录", "# 创作状态卡"]);
const meaningful4 = c4.split("\n").map(function (l) { return l.trim(); }).filter(Boolean).filter(function (l) { return !TEMPLATE_TITLES4.has(l) && !l.startsWith("（") && !l.startsWith("【"); });
...
const TEMPLATE_TITLES = new Set(["# 创作设定", "# 主要人物设定", "# 次要人物设定", "# 剧情大纲", "# 钩子记录", "# 创作状态卡"]);
const meaningful = c.split("\n").map(function (l) { return l.trim(); }).filter(Boolean).filter(function (l) { return !TEMPLATE_TITLES.has(l) && !l.startsWith("（") && !l.startsWith("【"); });
  ...
  ctx.tools.register({
    ...
    async execute(args, exec) {
      ...
      if (action === "add") {
        ...
        } else if (args?.category === "worldview" && /west|欧|西式|教廷|教堂|神甫|公爵/.test(String(args?.basis ?? ""))) {
          // 便捷：欧式基准自动带默认语用规范
          entry.speechStyle = {
...
Created 第${String(value.number).padStart(2, "0")}章 (${value.chars} chars)
    ...
    async execute(args, exec) {
      ...
      const reminder = "本章已创建（第" + String(number).padStart(2, "0") + "章）。【风格基线强制流程】动笔前请对照上方 baseline 六维 μ（完整基线可再调 novel_style_report 获取每维 σ 与容差带）；写完本章后必须调用 novel_style_check 对照基线——verdict 低于 high 或任一维度出带，需修正文风或说明情节需要，不得跳过。";
      return cleanOutput({ book, number, file: fileName, path: filePath, chars: text.length, baseline: baselineLine, reminder });
    }
  });
}
