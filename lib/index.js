...
import { RAW_WRITING_PROMPT, WORKFLOW_TEXT, BRIEF_TEXT, PROMPT_SCENES, promptTextFor } from "./prompts.js";
// v5.0.0：写作能力层——开写包（动笔前装配材料）与结构视图（伏笔/人物/情节线的跨章计算）
import { buildChapterBrief } from "./brief.js";
import { buildFixPlan, verifyFixPlan, markFixItem } from "./fixplan.js";
import { buildStoryGraph } from "./graph.js";
import * as embedding from "./embedding.js";
import { CACHE_VERSION, resolveAmbiguousCarriers, READ_LIMIT, READ_MAX_CHARS, parseChapterNumber, normalizeChapterKey, assert, requiredString, optionalString, optionalInt, sanitizeSegment, resolveRoot, novelsDir, bookDir, scanChapters, readTextFile, chapterStats, countTextLines, findChapter, extractKeywords, nextFreeChapterFile, bookNameFromFileName, bookNameFromContent, collectTextFiles, formatImport, ALL_TOOLS, TOOL_LABELS, detectCulture, detectGenre, detectTheme, FEATURE_DEFAULTS, featureEnabled, enrichSemanticImplicit, semanticFeatureEnabled, cropEmotion, toolEnabled, assertToolEnabled, stateFilePath, readStyleEnabled, readSentenceState, readSentenceStateSync, atomicWriteJson, withFileTx, clampStyleTolerance, writeSentenceState, effectiveSentenceAnalysis, makeStateRoutes, buildBriefLine, formatSentenceAnalysis, syncCreationProfileFiles, creationDir, CREATION_FILES, createOutlineTool, novelDataDir, metricChaptersCached, bookAnalysisCached, writeBookSemanticCache, plotsFile, readPlots, writePlots, plotKeywords, normalizePlotEntry, settingsFile, readSettings, noteRoot, writeSettings, normalizeSettingEntry, normalizeSettingList, readBookAllText, buildStyleAnchorPackage, parseEvidenceCounts, extractTopKeywords, summariesFile, readSummaries, cleanSummaryEntry, detectChapterBridge, formatBooks, formatChapters, formatRead, formatKeywords, cleanOutput } from "./core.js";
// v4.3.0：清掉本导入里 43 个"导入后零使用"的死符号（PLUGIN_VERSION/VIBE_*_PROTOTYPES/CHAPTER_EXTENSIONS/
// CJK_DIGITS/cjkToNumber 等，逐个 grep 确认全文件仅 import 行出现一次）——121 → 78 个符号。
// 注：creationDir / CREATION_FILES 当时确为零引用被清掉，随后跨区域裁决的"硬编码路径改常量"（大纲路径）
// 与"行数口径统一"（countTextLines）重新引入，故此处补回这三个符号并保留使用点。

const name = "novel-writer";
const inject = ["tools", "systemPrompt"];

// ---- v4.3.0：把包内 skills/novel-writing 真正注册给宿主（P1-5）----
// 旧版把 SKILL.md 放在包里却没有任何机制加载它：① 文件没有 YAML frontmatter，dsh-skill-filesystem
// 会直接忽略（"missing YAML frontmatter"）；② 包内 skills/ 不在宿主枚举的任何技能根下；③ 插件既没注册
// provider 也没在 cordis.patch.yml 里挂 root。于是默认「精简」档提示词里那句"详细规范见 novel-writing
// 技能"指向一个查不到的技能。现按官方 dsh-skill-badge 的做法自注册 provider：
// 不写用户目录、不依赖用户改配置，宿主没有 skills 服务时静默跳过（可选注入）。
const SKILL_NAME = "novel-writing";
const SKILL_DESCRIPTION = "中文网文写作助手（dsh-novel-writer 插件）：章节库浏览与阅读、句式/情感/风格基线分析、伏笔与设定表管理、章节摘要、连贯性审计（衔接/OOC/大纲走偏）、本地语义检索、原创模式创作资料。当用户要写/续写/改稿中文小说，或要看文风与情感量化报告时使用。";
const SKILL_WHEN_TO_USE = "用户提到小说、网文、章节、续写、改稿、文风、句式分析、伏笔、设定表、人物卡、大纲、章节摘要时";
const SKILL_BODY_URL = new URL("../skills/novel-writing/SKILL.md", import.meta.url);
const SKILL_INVOCATION = { modelInvocable: true, userInvocable: true };
const SKILL_RESOURCE_BASE = { kind: "directory", path: fileURLToPath(new URL("../skills/novel-writing/", import.meta.url)) };
/** 去掉 YAML frontmatter，只把正文交给宿主（name/description 走候选字段，避免正文里再出现一份）。 */
...
const skillProvider = {
  ...
  list: () => Promise.resolve([{
    ...
    source: "custom",
    provider: name,
    rank: 600, // 与宿主 BUNDLED_SKILL_RANK 同档；名字唯一，rank 仅在重名时参与裁决
  ...
  async get() {
    return {
      ...
      provider: name,
      resourceBase: SKILL_RESOURCE_BASE,
      content: stripSkillFrontmatter(await readFile(SKILL_BODY_URL, "utf8"))
    };
  }
};

// ---- v4.3.0 修复：开关裁剪 / 候选截断 / 分析报告缓存上限 ----

/**
 * v4.3.0：按 emotionCaveat / emotionComplexity 两个**互不牵连**的开关裁剪情感块。
 * 旧版直接调 core.js 的 cropEmotion（其返回体只有 dominant/scores/intensity/topWords/curve），
 * 于是 emotionCaveat=false 会把 quantification 一并剥掉，紧随其后的
 * `if (!emotionComplexity && emotion.quantification)` 恒为假——"关净化预警"连带杀掉了"情感量化"。
 * core.js 不在本次改动白名单内，故在裁剪前留存 quantification，再单独按 emotionComplexity 决定去留。
 */
function cropEmotionByFeatures(state, emotion) {
  if (!emotion) return emotion;
  if (featureEnabled(state, "emotionCaveat")) {
    // 净化开关为开：只在量化开关关闭时剔除 quantification（不重建对象，避免丢字段）
    if (!featureEnabled(state, "emotionComplexity")) delete emotion.quantification;
    return emotion;
  }
  const quantification = emotion.quantification;
  const cropped = cropEmotion(emotion);
  if (featureEnabled(state, "emotionComplexity") && quantification !== void 0) cropped.quantification = quantification;
  return cropped;
}

// ---- v4.3.0：语义缓存判据统一 + 语义风格空表文案细分 ----

/**
 * v4.3.0：语义门控——把"用户明确关闭语义功能"与"语义引擎瞬时不可用"两种不可用分开。
 * 判据（与 novel_style_report 既有实现逐字一致）：keepCached === (semOn && !engineReady)。
 *   semOn       用户是否仍开启语义隐性情感（semanticImplicit，含 semanticEmbedding 总开关）
 *   engineReady 引擎此刻是否可用（瞬时态：模型加载失败 / 30s 冷却窗口内为 false，恢复后仍可用）
 *   embOk       本次可做新推理（semOn && engineReady）
 *   keepCached  命中缓存时必须原样复用缓存里的 semanticImplicit / implicitResolved，
 *               并且**不得**把"清空后的结果"写回缓存——只有"用户没关、引擎却暂时起不来"才为真。
 *               v4.3.0 之前 novel_sentence_analysis 的命中路径没有这个区分，只要引擎此刻不可用
 *               就把缓存里的 semanticImplicit 抹成 null，恢复后要重跑 29 次原型推理 + 全索引检索。
 */
async function semanticGate(state) {
  const semOn = semanticFeatureEnabled(state, "semanticImplicit");
  const engineReady = await embedding.isAvailable();
  return { semOn, engineReady, embOk: semOn && engineReady, keepCached: semOn && !engineReady };
}

/**
 * v4.3.0：语义风格距离为空时的报告文案——按 lib/vibe.js semanticStyleDistances() 挂在返回数组上的
 * **非枚举** reason 区分失败通道（实测取值：no-engine / no-chunks / proto-embed-failed /
 * chunk-embed-failed / error / ok，另有"函数未运行"时该属性缺失）。
 * 旧版一律打印"（语义引擎不可用，跳过）"：引擎其实可用、只是本书太短或原型向量失败时误导用户。
 * reason 缺失或无法识别 → 回退旧通用文案（保持健壮）。文案面向作者、保留"跳过"语义、不暴露技术细节。
 */
function semanticSkipText(reason) {
  switch (reason) {
    case "no-engine": return "（语义引擎不可用，语义风格距离已跳过）";
    case "no-chunks": return "（本书文本太短，分不出语义段落，语义风格距离已跳过）";
    case "proto-embed-failed": return "（风格原型向量生成失败，语义风格距离已跳过）";
    case "chunk-embed-failed": return "（本书段落向量生成失败，语义风格距离已跳过）";
    case "error": return "（语义风格距离计算失败，已跳过）";
    case "ok": return "（本次未得到可比对的风格距离，已跳过）";
    default: return "（语义引擎不可用，跳过）"; // 原因缺失/未知：保持 v4.3.0 旧文案
  }
}

/** v4.3.0：连贯性审计候选的章节列表统一上限（人物缺失/缺场早已 slice(0,5)，其余几路漏了）。 */
const CANDIDATE_CHAPTER_LIMIT = 5;
/**
 * v4.3.0：截断候选章节列表并给出 disclosure 文案——旧版用语/客套/仪式/称谓四路把全书章节名
 * 全部塞进候选，几百章的书输出与落盘报告爆长（同函数内"人物缺失"已截断，口径不一致）。
 */
function capCandidateChapters(files) {
  const list = Array.isArray(files) ? files : [];
  return {
    chapters: list.slice(0, CANDIDATE_CHAPTER_LIMIT),
    note: list.length > CANDIDATE_CHAPTER_LIMIT ? `（共 ${list.length} 章，仅列前 ${CANDIDATE_CHAPTER_LIMIT}）` : ""
  };
}

/**
 * v4.3.0：分析报告缓存上限。缓存键含内容哈希与全部参数，每次改稿都会生成新文件，
 * 旧版既无 TTL 也无份数上限，<root>/.novel-writer/analysis 会无限膨胀。
 * 写入后按"书前缀"只保留最近 REPORT_CACHE_KEEP 份（按 mtime 降序，思路同 core.js reports 路由）。
 * 注意 <书>-full.json / <书>-chapters-metrics.json 是分析缓存（非报告），匹配不到、不会被删。
 */
const REPORT_CACHE_KEEP = 20;
async function pruneAnalysisReports(dir, book, kind) {
  try {
    const prefix = book + "-";
    const isReport = kind === "keywords"
      ? (name) => name.startsWith(prefix + "keywords-") && name.endsWith(".json")
      : (name) => /^[0-9a-f]{20}\.json$/.test(name.slice(prefix.length));
    const entries = await readdir(dir, { withFileTypes: true });
    const files = [];
    for (const entry of entries) {
      if (!entry.isFile() || !isReport(entry.name)) continue;
      try { files.push({ name: entry.name, mtime: (await stat(join(dir, entry.name))).mtimeMs }); } catch { /* 统计失败：忽略该文件 */ }
    }
    if (files.length <= REPORT_CACHE_KEEP) return;
    files.sort((a, b) => b.mtime - a.mtime);
    for (const stale of files.slice(REPORT_CACHE_KEEP)) {
      try { await rm(join(dir, stale.name), { force: true }); } catch { /* 删除失败留待下次清理 */ }
    }
  } catch { /* 目录不存在/不可读不阻塞主流程 */ }
}
...
function registerNovelSentenceAnalysis(ctx, config) {
  ctx.tools.register({
    ...
    description: "句式模式分析：统计某部作品（或单章）的句式分布（陈述/环境/心理/对话/疑问/反问/感叹/祈使/省略留白九类）、句式排列规律（转移、高频模板、段首段尾、按章节的压缩节奏序列）、段落结构、句长分布、情感曲线、风格指纹与节奏建议，用于快速掌握作者的写作习惯、主观情感并参考其叙事节奏。【问「这本书是怎么写的」时用它】——它与 novel_style_check（问「我这一章写得像不像」）、novel_style_report（问「这本书是什么风格」）共用同一套引擎与缓存，三者是不同的问题、不是重复的工具。受 UI 开关控制，可先用 novel_sentence_config 查看状态。",
    parameters: {
      type: "object",
      properties: {
    ...
    output: {
      ...
      // v4.3.0：brief=true 时 render 直接给精简摘要——core.js 的 formatSentenceAnalysis 从不读 value.brief，
      // 旧版仍把全量【句式分布】…【AI 解读提示】塞给模型，"省 token"名不副实
      render: (_args, value) => [{
        type: "text",
        text: typeof value.brief === "string" && value.brief !== ""
          ? `<path>novels/${value.book}</path>
<type>novel-sentence-analysis</type>
<content>
【句式分析·精简】${value.brief}
（brief=true：完整句式分布/情感量化/节奏建议请省略 brief 重新调用）
</content>`
          : formatSentenceAnalysis(value)
      }]
    },
    async execute(args, exec) {
      ...
      // v5.0.0（降本）：精简工作流开启且调用方未显式指定 brief 时，句式分析默认走简报（完整版约 2k 字符）。
      // 命中与未命中两条路径共用同一个判据，避免口径漂移。
      const sentenceBrief = args?.brief === true || (args?.brief === void 0 && isLeanWorkflow());
      let cachedHit = null;
      if (!fresh) {
        try {
      ...
      if (cachedHit) {
        ...
        // v4.3.0：与 novel_style_report 同一套判据（共用 semanticGate）——只有"用户明确关闭语义功能"
        // 才允许清缓存；"引擎瞬时不可用"必须保留并复用缓存里的 semanticImplicit
        const semGate = await semanticGate(state);
        await enrichSemanticImplicit(state, root, book, cachedHit, exec, chapterArg !== void 0 ? selected[0].file : void 0);
        // v4.3.0：引擎瞬时不可用时 enrichSemanticImplicit 会按"引擎不可用"清掉结果——这里把缓存值挂回，
        // 本次报告仍显示隐性情绪，且 siBefore===siAfter 使下面跳过写回（缓存不再被抹成 null）
        if (semGate.keepCached && siBefore && cachedHit?.emotion?.quantification && !cachedHit.emotion.quantification.semanticImplicit) {
          cachedHit.emotion.quantification.semanticImplicit = siBefore;
        }
        ...
        // v4.3.0：emotionCaveat 与 emotionComplexity 各自独立生效（旧版 cropEmotion 连带剥掉 quantification）
        cachedHit.emotion = cropEmotionByFeatures(state, cachedHit.emotion);
        if (sentenceBrief && cachedHit.categories && cachedHit.lengths && cachedHit.emotion) {
      ...
      try {
        ...
        // v4.3.0：未命中 / fresh 路径同样不得抹掉缓存里的语义结果。
        // 场景：调用方带 fresh:true 强制重算，而此刻语义引擎恰好不可用（用户并未关闭该功能）——
        // 旧版会把"算不出 semanticImplicit"的结果写回**同一个缓存文件**（缓存键不含 fresh），
        // 于是之前算好的语义结果被永久丢掉。此处把旧缓存里的同名字段并回**落盘副本**：
        // 缓存键含内容指纹，同键即同内容，旧值仍然有效；本次返回值不受任何影响（仍是"未算出"）。
        const persist = { ...result, cachedAt: new Date().toISOString(), ver: CACHE_VERSION };
        if (result.emotion?.quantification && !result.emotion.quantification.semanticImplicit && (await semanticGate(state)).keepCached) {
          try {
            const prev = JSON.parse(await readFile(reportFile, "utf8"));
            const prevSi = prev && prev.ver === CACHE_VERSION && prev.emotion && prev.emotion.quantification
              ? prev.emotion.quantification.semanticImplicit
              : void 0;
            if (prevSi) {
              persist.emotion = { ...result.emotion, quantification: { ...result.emotion.quantification, semanticImplicit: prevSi } };
            }
          } catch { /* 旧缓存不存在/损坏：无需并回 */ }
        }
        await atomicWriteJson(reportFile, persist);
        // v4.3.0：报告缓存上限——按书只保留最近 REPORT_CACHE_KEEP 份（旧版无上限，改稿即新增文件）
        await pruneAnalysisReports(cacheDir, book, "sentence");
      } catch { /* 缓存写失败不影响结果 */ }
      // v4.3.0：两个情感开关独立生效（旧版 cropEmotion 连带剥掉 quantification，complexity 分支成死代码）
      result.emotion = cropEmotionByFeatures(state, result.emotion);
      if (sentenceBrief && result.categories && result.lengths && result.emotion) {
...
function registerNovelSentenceConfig(ctx, config) {
  ctx.tools.register({
    ...
    description: "查看或修改句式模式分析的开关状态：enabled（分析功能是否可用）、autoAnalyze（分析作品时是否主动使用）、systemPromptMode（系统提示词档位 off/brief/full）、promptScene（提示词场景 general/writing/revising/auditing/setup，决定完整档下注入哪一套流程）与 leanWorkflow（精简工作流：打开后只按需调用工具，省 token）。",
    parameters: {
      type: "object",
      properties: {
        ...
        // v5.0.0：提示词"档位 × 场景"两个正交维度。档位决定注入多少，场景决定注入什么。
        systemPromptMode: { type: "string", enum: ["off", "brief", "full"], description: "系统提示词档位：off=不注入 / brief=精简（只说明有这套工具） / full=完整（按场景注入对应流程）。" },
        promptScene: { type: "string", enum: [...PROMPT_SCENES], description: "提示词场景（仅 full 档生效）：general=通用写作工作流（默认，等价于 4.x 行为） / writing=写新章 / revising=改稿 / auditing=审计（只体检不改稿） / setup=建资料（只维护设定与创作资料）。" },
        // v5.1.0（降本）：精简工作流——打开后注入最简档，去掉自检/复测/登记的全部催促
        leanWorkflow: { type: "boolean", description: "精简工作流（默认 false）：true=注入最简提示词（优先于档位与场景），只保留「工具按需调用」一条原则，报告类工具未显式传 brief 时默认走精简输出。适合成本敏感的长篇。" },
    ...
    output: {
      schema: {
        ...
        properties: {
          ...
          enabled: { type: "boolean" },
          autoAnalyze: { type: "boolean" },
          // v5.0.0：提示词档位与场景（此前这对开关只在侧边栏可见，模型侧看不到自己的注入状态）
          systemPromptMode: { type: "string" },
          promptScene: { type: "string" },
          // v5.1.0（降本）：精简工作流（与 promptScene 同级；仅接受布尔值）
          leanWorkflow: { type: "boolean" },
          source: { type: "string" },
...
系统提示词: 档位=${value.systemPromptMode ?? "brief"} 场景=${value.promptScene ?? "general"}${(value.systemPromptMode ?? "brief") === "full" && (value.promptScene ?? "general") !== "general" ? "（按场景注入）" : ""}
精简工作流(leanWorkflow): ${value.leanWorkflow === true ? "开（优先于档位/场景，只按需调用工具）" : "关"}
来源: ${value.source}
...
${featLines.join("\n")}
    ...
    async execute(args) {
      ...
      if (action === "set") {
        ...
        // v5.0.0：提示词档位与场景（非法值直接忽略；general 是显式合法值，用于改回通用档）
        if (["off", "brief", "full"].includes(args?.systemPromptMode)) patch.systemPromptMode = args.systemPromptMode;
        if (PROMPT_SCENES.includes(args?.promptScene)) patch.promptScene = args.promptScene;
        // v5.1.0（降本）：精简工作流（只接受布尔值；非布尔静默忽略，与 core.writeSentenceState 同口径）
        if (typeof args?.leanWorkflow === "boolean") patch.leanWorkflow = args.leanWorkflow;
        ...
        if (Object.keys(patch).length > 0) {
          await writeSentenceState(patch);
          // v4.3.0：写后重读 state——writeSentenceState 的返回值继承写前快照（首写时 exists:false），
          // 而 effectiveSentenceAnalysis 以 exists===true 为门，旧版首次 set 会返回与刚落盘状态相反的
          // enabled/source（盘上 enabled:false，返回值却是 config 的 true + "插件 config"）
          current = await readSentenceState();
          updated = true;
        }
      }
      ...
      if (updated && args?.creationProfiles !== void 0) {
        ...
        if (syncRoot) {
          try { await syncCreationProfileFiles(syncRoot, args.creationProfiles, current.creationProfiles); } catch { /* 文件同步失败不影响设定保存 */ }
            ...
            return { file: stateFilePath(), enabled: after.enabled, autoAnalyze: after.autoAnalyze, source, updated, systemPromptMode: current?.systemPromptMode ?? "brief", promptScene: current?.promptScene ?? "general", leanWorkflow: current?.leanWorkflow === true, tools, features, styleTolerance, creationProfile, creationProfiles, embedding: { available: embAvailable, loaded: embStatus.loaded, error: embError } };
    }
  });
}
function registerNovelStyleCheck(ctx, config) {
  ctx.tools.register({
    ...
    description: "风格自检：对比某章节与全书其他章节的风格指纹（句式分布/句长/情绪），输出相似度与偏差清单。【问「我这一章写得像不像全书」时用它】——要可执行的改稿待办请接着用 novel_fix_plan，要全书风格画像用 novel_style_report，要看写作手法用 novel_sentence_analysis。",
    parameters: {
      type: "object",
      properties: {
        book: { type: "string", description: "书名。" },
    ...
    output: {
      ...
      render: (_args, value) => {
        ...
        if (Array.isArray(value.fixAnchors) && value.fixAnchors.length > 0) {
          // v5.1.0（降本）：抬头从"逐句对比…只修跑偏部分"改为条件式——只有确实跑偏时才用锚段对照，
          // 且明确"改那几处"，避免模型把正常波动也当成必须修的偏差。
          lines.push("", "【对照参考·原著锚段】若确实跑偏（偏离明显或读起来别扭），可对照这几段原著的写法只调整那几处，不要整章重写：");
        ...
        if (value.metric && value.metric.enabled) {
          ...
          for (const v of value.metric.verdicts || []) {
            ...
            // v4.3.0：μ=0 的维度按绝对尺度判定（devPct 为 null），单独表述，不打印 "null%"
            const devText = v.devPct === null || v.devPct === void 0
              ? "基线为 0，按绝对尺度判定"
              : (v.devPct > 0 ? "+" : "") + v.devPct + "%";
            lines.push(`  ${v.label}: 本章 ${v.value} vs 基线 ${v.mu}（${devText}，${mark}）`);
          }
          // v5.1.0（防僵化）：删掉句尾「——请对照基线修正后再续写」——它与同一份输出里的建议段自相矛盾
          // （建议段说"轻微出带属正常波动，不必逐项消除"）；模型会听更早、更硬的那半句，于是反复打磨。
          if (value.metric.outCount > 0) lines.push("  ⚠ " + value.metric.summary);
          // v4.3.0：即使没有出带维度，也要披露"因基线为 0 未做相对判定"的维度（旧版一律不打印）
          else if ((value.metric.skippedDims || []).length > 0) lines.push("  · " + value.metric.summary);
    ...
    async execute(args, exec) {
      ...
      advice = "续写风格偏差：" + diffs.slice(0, 4).map((d) => d.dimension + (noteMap[d.note] || d.note)).join("、") + "。**先判断这是不是有意为之**：章节类型不同（抒情段 / 动作段 / 对话段）本来就会偏离全书均值——确属风格选择就保留；确实跑偏时，可对照 fixAnchors 中的原著锚段只调整那几处（句式/修饰/节奏），不要整章重写，也不要为对齐数字改文。";
      }
      ...
      try {
        ...
        if (b.chapterCount > 0) {
          ...
          metric = { enabled: true, baseline: b.baseline, metrics: targetMetrics, verdicts: judge.verdicts, outCount: judge.outCount, skippedDims: judge.skippedDims, summary: judge.summary };
      ...
      if (metric && metric.enabled && metric.outCount > 0) {
        ...
        advice = diffs.length === 0
          ? "句式指纹与全书高度一致；六维层面：" + dimNote + "——**单一维度出带属正常波动**（带宽 = 作者自身波动的 1.5σ），只有偏离达到 2 倍容差以上、或那一处读起来确实别扭时才需要调整，不要为对齐数字改写文本。"
          : advice + "另（六维）：" + dimNote + "——同上，轻微出带属正常波动，不必逐项消除。";
      }
      return cleanOutput({
        book,
        chapter: target.file,
...
/**
 * v5.0.0：开写包——动笔前一次性装配「写这一章需要的一切材料」。
 *
 * 设计动机：此前这些材料分散在 6~8 次工具调用里，靠系统提示词叮嘱模型去取；
 * 而**漏读正是文风漂移与设定矛盾的头号来源**。本工具把它压成一次调用，从结构上消灭漏读。
 * 材料来源与 novel_new_chapter 附带的「基线 + 锚段 + 骨架」同源，这里是抽离与扩容。
 */
function registerNovelChapterBrief(ctx, config) {
  ctx.tools.register({
    name: "novel_chapter_brief",
    description: "开写包：一次调用装配出「写这一章需要的一切」——上一章结尾原文（承接口）、本章大纲方向、相关人物卡、待回收伏笔（含埋设距离）、世界观用语规范、六维风格基线、原著锚段与句式骨架、本章禁用清单与开写清单。只读不写盘（仅复用既有分析缓存）。**写新章前应先调它**，而不是逐个工具去翻。",
    parameters: {
      type: "object",
      properties: {
        book: { type: "string", description: "书名。" },
        chapter: { type: "string", description: "目标章：next（默认，= 当前最大章号 + 1，文件可尚未创建）/ 章号 / 文件名 / 标题子串。" },
        budget: { type: "string", enum: ["compact", "full"], description: "材料档位：compact（默认，各字段按小额截断以省上下文）/ full（锚段与骨架给全）。" },
        root: { type: "string", description: "章节库根目录。" }
      },
      required: ["book"],
      additionalProperties: false
    ...
    output: {
      schema: {
        ...
        properties: {
          book: { type: "string" },
          chapter: { type: "object", additionalProperties: true, description: "{ file, number, title }；无法唯一确定目标章时整键省略（原因见 degraded）。" },
          isNext: { type: "boolean" },
          anchor: { type: "string" },
          previousHook: { type: "string" },
          outlineDirection: { type: "string" },
          characters: { type: "array", items: { type: "object", additionalProperties: true }, description: "相关人物卡：{ name, role, description, mentions, sources[], offstage? }" },
          openPlots: { type: "array", items: { type: "object", additionalProperties: true }, description: "未回收伏笔：{ id, content, priority, type, firstChapter?, distance?, ... }，按 优先级 → distance 降序" },
          worldview: { type: "object", additionalProperties: true, description: "{ basis, bannedWords, recommended, speechStyle }；未登记时省略" },
          baseline: { type: "object", additionalProperties: true, description: "六维基线 { <metric>: { mu, sigma, recTol } }；书太短等情况下省略" },
          anchors: { type: "array", items: { type: "object", additionalProperties: true }, description: "原著锚段 [{ label, text }]" },
          skeletons: { type: "array", items: { type: "object", additionalProperties: true }, description: "句式骨架 [{ type, text }]" },
          lastVerdict: { type: "string" },
          avoid: { type: "array", items: { type: "object", additionalProperties: true }, description: "本章禁用清单 [{ kind, detail }]" },
          plan: { type: "object", additionalProperties: true, description: "{ previousState, goal, checklist[] }" },
          degraded: { type: "array", items: { type: "string" } }
        },
        required: ["book", "isNext", "anchor", "previousHook", "outlineDirection", "characters", "openPlots", "anchors", "skeletons", "lastVerdict", "avoid", "plan", "degraded"]
      },
      render: (_args, value) => {
        const out = [`<path>novels/${value.book}</path>`, "<type>novel-chapter-brief</type>", "<content>", ""];
        out.push(value.chapter
          ? `目标：第 ${value.chapter.number ?? "?"} 章《${value.chapter.title || "（无标题）"}》${value.isNext ? "（尚未创建，文件名推导为 " + value.chapter.file + "）" : ""}`
          : "目标：无法唯一确定要写哪一章（见文末降级说明）");
        if (value.outlineDirection) out.push("", "【本章方向】" + value.outlineDirection);
        if (value.previousHook) out.push("", "【上一章钩子】" + value.previousHook);
        if (value.anchor) out.push("", "【上一章结尾原文·承接口】", value.anchor);
        if (value.openPlots.length > 0) {
          out.push("", "【待回收伏笔】");
          for (const p of value.openPlots) out.push(`  - [${p.id}] ${p.content}（${p.priority}｜第 ${p.firstChapter ?? "?"} 章埋下，已过 ${p.distance ?? "?"} 章）`);
        }
        if (value.characters.length > 0) {
          out.push("", "【相关人物】");
          for (const c of value.characters) out.push(`  - ${c.name}${c.offstage === true ? "（已退场，勿写其出场）" : ""}：${c.description || c.role || ""}`);
        }
        if (value.worldview) {
          const w = value.worldview;
          const banned = Array.isArray(w.bannedWords) ? w.bannedWords.slice(0, 10).join("、") : "";
          out.push("", "【世界观用语】" + [w.basis, banned ? "禁用：" + banned : ""].filter(Boolean).join("｜"));
        }
        if (value.baseline) {
          const mu = Object.keys(value.baseline).map((k) => `${k} μ=${value.baseline[k].mu}`).join("  ");
          out.push("", "【风格基线】" + mu);
        }
        if (value.anchors.length > 0) {
          out.push("", "【原著锚段·照这个味道写】");
          for (const a of value.anchors) out.push(`  [${a.label}] ${a.text}`);
        }
        if (value.skeletons.length > 0) {
          out.push("", "【句式骨架·按这个形状造句】");
          for (const s of value.skeletons) out.push(`  [${s.type}] ${s.text}`);
        }
        if (value.lastVerdict) out.push("", "【上一章自检】" + value.lastVerdict);
        if (value.avoid.length > 0) {
          out.push("", "【本章禁用清单】");
          for (const a of value.avoid) out.push(`  - ${a.kind}：${a.detail}`);
        }
        out.push("", "【开写清单】");
        for (const item of value.plan.checklist) out.push("  □ " + item);
        if (value.degraded.length > 0) {
          out.push("", "【降级/提示】");
          for (const d of value.degraded) out.push("  - " + d);
        }
        out.push("", "</content>");
    ...
    async execute(args, exec) {
      await assertToolEnabled(config, "novel_chapter_brief");
      const book = sanitizeSegment(requiredString(args, "book"), "book");
      const chapterArg = optionalString(args, "chapter") ?? "next";
      const budget = args?.budget === "full" ? "full" : "compact";
      const root = resolveRoot(config, args, exec);
      // featState 用于让"上一章六维判定"沿用侧边栏保存的容差（与 novel_style_check 同口径）
      const brief = await buildChapterBrief(root, book, chapterArg, { budget, featState: await readSentenceState() }, exec);
      return dropNullDeep(brief);
    }
  });
}

/**
 * v5.0.0：改稿台——把风格诊断从「数据」变成「按优先级排好的待办」，并支持改完复测。
 *
 * 与 novel_style_check 的分工：style_check 给判定与偏差清单（数据），本工具给行动（清单 + 复测三态）。
 * **边界**：hint 只给改写方向，绝不生成改写后的正文——生成是宿主模型的事。
 */
function registerNovelFixPlan(ctx, config) {
  ctx.tools.register({
    name: "novel_fix_plan",
    description: "改稿台：把风格诊断变成按优先级排好的待办清单——每条含原句位置（行号区间 + 原文）、当前值与目标值、可参照的原著锚段、以及改写方向。action=plan 生成清单 / verify 对当前正文复测（看哪些已回到带内）/ mark 标记单项（需 itemId + state）。**只给方向不给句子**：本插件不生成正文。",
    parameters: {
      type: "object",
      properties: {
        book: { type: "string", description: "书名。" },
        chapter: { type: "string", description: "要改的章节（章号 / 文件名 / 标题子串）。" },
        action: { type: "string", enum: ["plan", "verify", "mark"], description: "plan=生成待办清单（默认）；verify=对当前正文复测；mark=标记单项（需同时传 itemId 与 state）。" },
        itemId: { type: "string", description: "mark 时必填：要标记的待办 id（取自 plan 返回的 items[].id）。" },
        state: { type: "string", enum: ["done", "skip"], description: "mark 时必填：done=已处理 / skip=跳过。" },
        root: { type: "string", description: "章节库根目录。" }
      },
      required: ["book", "chapter"],
      additionalProperties: false
    ...
    output: {
      schema: {
        ...
        properties: {
          book: { type: "string" },
          action: { type: "string" },
          chapter: { type: "object", additionalProperties: true, description: "{ file, number, title }" },
          items: {
            type: "array",
            items: {
              type: "object",
              ...
              properties: {
                ...
                locate: {
                  ...
                  required: ["lineStart", "lineEnd", "excerpt"]
                },
                current: { type: "string" },
                target: { type: "string" },
                anchor: { type: "string" },
              ...
              required: ["id", "type", "locate", "current", "target", "anchor", "severity", "effort", "hint"]
            }
          },
          checked: {
            type: "array",
            items: {
              ...
              properties: {
                id: { type: "string" },
                type: { type: "string" },
                status: { type: "string", enum: ["resolved", "pending", "new"] },
                detail: { type: "string" }
              },
              required: ["id", "type", "status", "detail"]
            }
          },
        ...
        required: ["book", "action", "summary"]
      },
      render: (_args, value) => {
        const out = [`<path>novels/${value.book}</path>`, "<type>novel-fix-plan</type>", "<content>", "", value.summary];
        if (Array.isArray(value.items) && value.items.length > 0) {
          out.push("", `待办 ${value.items.length} 条（按 严重度 → 难度 → 行号 排序）：`);
          // v5.0.0（降本）：明示"分批 + 单次复测"——旧版没有这句，模型会逐条改 + 逐条 mark + 反复 verify，
          // 实测一次改稿 30+ 次调用（每次重发整章），这也是"改稿比单写还贵"的直接原因。
          out.push("  处理建议：**分批改**（同一段落/同一类型的多条合并成一次修改），一轮处理最严重的 3~5 条即可；不必逐条 mark，改完最多 verify 一次。指标类（抽象度/留白/不确定性/情感过直）先判断再动手：抒情、心理、留白段属正常写法，读起来不别扭就保留。");
          for (const it of value.items) {
            const loc = it.locate.lineStart > 0 ? `第 ${it.locate.lineStart}${it.locate.lineEnd !== it.locate.lineStart ? "-" + it.locate.lineEnd : ""} 行` : "全章层面";
            // v5.0.0 修正：把 itemId 渲染出来。此前 mark 需要的 id 只存在于返回体/落盘文件里，
            // 模型要 mark 必须先自己去读 planFile 才能拿到 id——清单里直接给，少一步往返。
            out.push(`  - [${it.type}] 严重度 ${it.severity} / 难度 ${it.effort}（${loc}）`);
            if (it.id) out.push(`      id：${it.id}`);
            if (it.current) out.push(`      现状：${it.current}${it.target ? "　目标：" + it.target : ""}`);
            if (it.locate.excerpt) out.push(`      原句：${it.locate.excerpt}`);
            out.push(`      方向：${it.hint}`);
            if (it.anchor) out.push(`      锚段：${it.anchor}`);
          }
        }
        if (Array.isArray(value.checked) && value.checked.length > 0) {
          out.push("", "复测：");
          for (const c of value.checked) out.push(`  - [${c.status}] ${c.type}：${c.detail}`);
        }
        if (value.item) out.push("", `已标记 ${value.item.id ?? ""} → ${value.item.state ?? ""}`);
        if (value.planFile) out.push("", "清单文件：" + value.planFile);
        out.push("", "提示：hint 只是改写方向，正文由你（宿主模型）来写。", "</content>");
        return [{ type: "text", text: out.join("\n") }];
      }
    },
    async execute(args, exec) {
      await assertToolEnabled(config, "novel_fix_plan");
      const book = sanitizeSegment(requiredString(args, "book"), "book");
      const chapterArg = requiredString(args, "chapter");
      const action = ["plan", "verify", "mark"].includes(args?.action) ? args.action : "plan";
      const root = resolveRoot(config, args, exec);
      // mark 只写落盘清单、不动正文，也不依赖六维引擎；plan/verify 依赖引擎，门禁与 novel_style_check 对齐
      if (action === "mark") {
        const itemId = requiredString(args, "itemId");
        const itemState = ["done", "skip"].includes(args?.state) ? args.state : void 0;
        // v5.0.0 修正：区分"没传"与"传了非法值"——旧文案在传了非法值时也说"需要 state 参数"，容易误判。
        assert(itemState !== void 0, args?.state === void 0
          ? 'novel_fix_plan mark 缺少 state 参数：只能是 "done"（已改）或 "skip"（跳过不改）'
          : 'novel_fix_plan mark 的 state 只能是 "done"（已改）或 "skip"（跳过不改），收到：' + JSON.stringify(args.state));
        const r = await markFixItem(root, book, chapterArg, itemId, itemState, {}, exec);
        return dropNullDeep({ book, action, summary: `已把待办 ${itemId} 标记为 ${itemState}（正文与判定不受影响，只记录处置状态）`, ok: r.ok, item: r.item });
      }
      const state = await readSentenceState();
      const effective = effectiveSentenceAnalysis(config, state);
      assert(effective.enabled, "写作助手功能（句式模式分析）当前已关闭，可在侧边栏「写作助手功能」面板开启后再用改稿台。");
      if (action === "verify") {
        const r = await verifyFixPlan(root, book, chapterArg, {}, exec);
        return dropNullDeep({ ...r, action });
      }
      const r = await buildFixPlan(root, book, chapterArg, {}, exec);
      return dropNullDeep({ ...r, action });
    }
  });
}
...
function registerNovelStyleReport(ctx, config) {
  ctx.tools.register({
    ...
    description: "风格画像报告：聚合 6 维测量数据（指纹/词汇/题材/情感/氛围 12 轴/语义距离）供 AI 判断风格。【问「这本书是什么风格」时用它】——它给的是全书画像而非单章判定（那是 novel_style_check），也不是写作手法拆解（那是 novel_sentence_analysis）。",
    parameters: {
      type: "object",
    ...
    async execute(args, exec) {
      ...
      try {
        // v4.3.0：门控抽成共用 semanticGate（判据不变）——novel_sentence_analysis 命中路径现在用同一判据
        // v4.3.0：引擎可用性单独探测——"用户关掉语义功能"（应清缓存）与"引擎瞬时不可用"（不该清）必须分开处理
        const semGate = await semanticGate(featState);
        const semOn = semGate.semOn;
        const embOk = semGate.embOk;
        ...
        // v4.3.0：引擎瞬时不可用时沿用缓存值（旧版置 null 后写回缓存＝把已裁决结果抹掉）
        let implicitResolved = semOn ? analysisCache.implicitResolved : null;
        ...
        // v4.3.0：引擎瞬时不可用时 enrichSemanticImplicit 会按"引擎不可用"清掉结果——这里把缓存值挂回，
        // 使本次报告与缓存保持一致（引擎恢复后仍可复用，不必重扫全书）
        if (semGate.keepCached && analysisCache.semantic && analysis.emotion?.quantification && !analysis.emotion.quantification.semanticImplicit) {
          analysis.emotion.quantification.semanticImplicit = analysisCache.semantic;
        }
        // v4.0.0：语义结果写回全书分析缓存（含内容指纹）；功能关闭/引擎不可用时写回即清除旧语义残留
        // v4.3.0：区分两种"不可用"——用户关闭语义功能→照旧写回清除；引擎瞬时不可用→跳过写回，保留缓存
        // v4.3.0：该表达式原样抽为 semGate.keepCached === (semOn && !engineReady)，行为不变
        if (analysis.emotion?.quantification && !semGate.keepCached) {
      ...
      // 组装报告文本
      // v4.3.0：报告组装前套用与 novel_sentence_analysis 同一套开关裁剪——
      // 旧版无论 emotionCaveat / emotionComplexity 开关如何，都照常输出 raw↔clean 对照、Δ/V/C 与隐性情绪，
      // 与 SKILL.md「关闭后对应输出消失」的承诺相反。
      const caveatOn = featureEnabled(featState, "emotionCaveat");
      const complexityOn = featureEnabled(featState, "emotionComplexity");
      const emotionView = cropEmotionByFeatures(featState, emotion);
      const q = complexityOn ? (emotion.quantification || {}) : {};
      ...
      // v4.3.0：caveat 关 → 不输出 raw↔clean 对照；complexity 关 → 不输出 Δ/V/C、意象与隐性情绪
      if (caveatOn) {
        lines.push("  表面 " + (emotion.dominant || "-") + " → 真实 " + (emotion.cleanDominant || "-") + "（置信度 " + (emotion.confidence || "-") + "）");
      } else {
        lines.push("  主导情绪 " + (emotion.dominant || "-") + "（情感净化预警已关闭，本行仅给规则主导情绪）");
      }
      if (complexityOn) {
        ...
        lines.push("  情感量化已关闭（emotionComplexity=false，不输出 Δ/V/C 与量化派生项）");
      }
      lines.push("");
      ...
      } else lines.push("  " + semanticSkipText(semantic?.reason)); // v4.3.0：按失败通道细分（缺失/未知 reason 回退旧文案）
      lines.push("");
      ...
      if (typeof args?.aiJudgment === "string" && args.aiJudgment.trim().length > 0) {
        ...
        try {
          ...
          // v4.3.0：改用 atomicWriteJson（本文件唯一直接 writeFile 目标文件的地方）——
          // 并发写同一本书的判断或进程中断会留下半截 JSON，后续 action:"get" 直接解析失败
          await atomicWriteJson(reportFile, {
            book,
            ...
            emotion: emotionView.cleanDominant ?? null, // v4.3.0：caveat 关闭时不落净化值（与报告同口径）
      ...
      // v5.0.0（降本）：精简工作流开启且调用方没显式指定 brief 时，默认走精简输出（省掉 2k+ 字符的报告全文）
      const briefMode = args?.brief === true || (args?.brief === void 0 && isLeanWorkflow());
      ...
      return cleanOutput({
        ...
        // v4.3.0：emotion 取开关裁剪后的视图——caveat 关闭时 cleanDominant 已剥，cleanOutput 会丢掉该键
        dimensions: briefMode ? void 0 : { fingerprint: analysis.fingerprint, culture: result.culture, genre: result.genre?.dominant, theme: result.theme?.dominant, emotion: emotionView.cleanDominant, vibe: Object.fromEntries(vibe.axes.slice(0, 12).map((a) => [a.name, a.score])) },
...
function registerNovelSummary(ctx, config) {
  ctx.tools.register({
    ...
    async execute(args, exec) {
      ...
      // v4.3.0：读盘后立即套用 cleanSummaryEntry 白名单（与 list/get 同口径）——旧版把磁盘原文直接交给
      // findIndex(s => s.chapter)/sort(a.chapter)，脏条目（null/字符串）会抛 TypeError 让整次调用失败；
      // settings/plots 早有 normalizeSettingList/normalizePlotEntry 守卫，summary 一直缺
      const summaries = (await readSummaries(root, book)).map(cleanSummaryEntry).filter(Boolean);
      let message = "";
      if (action !== "list") {
        const chapterArg = optionalString(args, "chapter");
...
function registerNovelContinuityCheck(ctx, config) {
  ctx.tools.register({
    ...
    async execute(args, exec) {
      ...
      const candidates = [];
      // v4.3.1（P0）：同章号多文件检测。findChapter 现在会在章号命中多个文件时拒绝执行（不再静默读错稿），
      // 所以在审计里先把撞号清单摆出来，否则用户只看到"读取失败"却不知道哪几章撞了。
      const dupByNumber = new Map();
      for (const c of chapters) {
        if (c.number === void 0) continue;
        const list = dupByNumber.get(c.number) ?? [];
        list.push(c.file);
        dupByNumber.set(c.number, list);
      }
      for (const [number, files] of dupByNumber) {
        if (files.length > 1) {
          candidates.push({
            type: "同章号多文件",
            detail: `第 ${number} 章有 ${files.length} 个文件共用同一章号：${files.join("、")}——按章号读取会被拒绝（需改用文件名指定）`,
            chapters: files
          });
        }
      }
      ...
      if (args?.outline === true) {
        const outlinePath = join(creationDir(root, book), CREATION_FILES.outline); // v4.3.0：改用常量拼路径（与 core.js 同源），避免目录改名后静默失配
        // v4.3.0：大 try 缩到只包"大纲本身"的读取——旧版把循环内"单章读失败"（被删/占用/权限）
        // 也一并吞掉，统一误报成"该书无创作资料"，按提示去 init 大纲后依旧失败
        let outlineText;
        try {
          outlineText = await readTextFile(outlinePath, exec);
        } catch {
        ...
        for (const l of directionLines) {
          ...
          let text;
          try {
            text = (await readTextFile(join(dir, ch.file), exec)).slice(0, 600);
          } catch {
            skipped += 1; // v4.3.0：单章读失败只跳过该章并计数，不再把整次大纲对照判成"无创作资料"
            continue;
          }
          const grams = new Set();
        ...
        const skipNote = skipped > 0 ? "（跳过 " + skipped + " 章读取失败）" : "";
        return { book, action: "大纲对照", candidates: out, advice: (out.length === 0 ? "各章与大纲方向基本一致。" : "发现 " + out.length + " 章可能与大纲方向偏离——逐条核对（偏离不一定是错，但要有意为之）。") + skipNote };
      }
      ...
      for (const { file, text } of chapterTexts) {
        // v4.3.0：字符类与下方解析侧 formValue 对齐（补 零/两）——旧版扫描侧缺这两个字，
        // 「两万」扫不到、解析侧的 零/两 分支成死代码，于是「两万」配不上「2万」（同一数量被判成两种写法）
        for (const m of text.matchAll(/\d+\s*(?:万亿|千万|百万|十万|[万亿千百])|[零一二两三四五六七八九十]{1,2}(?:万亿|千万|百万|十万|[万亿千百])/g)) {
        ...
        for (const group of byValue.values()) {
          ...
          for (let i = 0; i < group.length && emitted < 20; i += 1) {
            for (let j = i + 1; j < group.length && emitted < 20; j += 1) {
              const capped = capCandidateChapters([...new Set(group[i].chs.concat(group[j].chs))]); // v4.3.0：统一截断
              candidates.push({ type: "数字口径", detail: "「" + group[i].form + "」与「" + group[j].form + "」指同一数量但写法不一致，请统一口径" + capped.note, chapters: capped.chapters });
              emitted += 1;
            }
          }
        }
      ...
      for (const word of styleRule.bannedWords) {
        ...
        if (hits.length > 0) {
          ...
          const capped = capCandidateChapters(hits); // v4.3.0：与人物缺失同口径截断
          candidates.push({
            type: "用语冲突",
            detail: `「${word}」×${hits.length}章 与当前文化基准「${ruleCultureName}」不符${rec}${worldviewEntry ? "" : "（未登记 worldview，使用默认基准；可用 novel_settings detect 自动判断）"}${capped.note}`,
      ...
      if (speech && Array.isArray(speech.honorBad)) {
        ...
        for (const [word, files] of honorHits) {
          ...
          const capped = capCandidateChapters(files); // v4.3.0：统一截断（旧版把全书章节名全塞进候选）
          candidates.push({ type: "语用冲突·客套", detail: `「${word}」×${files.length}章 属于「${speechCultureName}」禁用表达${rec}${capped.note}`, chapters: capped.chapters }); // v4.0.0：标注语用规范条目名
        }
      }
      if (speech && Array.isArray(speech.ritualBadPatterns)) {
        for (const pattern of speech.ritualBadPatterns) {
          ...
          if (ritualHits.length > 0) {
            const capped = capCandidateChapters(ritualHits); // v4.3.0：统一截断并披露总数
            candidates.push({
              type: "语用冲突·仪式",
              detail: `检测到仪式类表达（规范：${speech.ritualGoodNote || "见 worldview 设定"}）${capped.note}`,
              chapters: capped.chapters
            });
          }
        }
      ...
      if (speech && typeof speech.title === "string") {
        ...
        if (/不用.{0,8}小姐|小姐.{0,8}(禁用|不用)|禁用.{0,8}小姐|不称.{0,6}小姐/.test(speech.title)) {
          ...
          if (count > 0) {
            const capped = capCandidateChapters(titleHits); // v4.3.0：统一截断并披露总数
            candidates.push({ type: "语用冲突·称谓", detail: `「XX小姐」×${count} 与称谓规范不符（${speech.title}）${capped.note}`, chapters: capped.chapters });
          }
        }
      }
...
/**
 * v5.0.0：把对象里值为 null/undefined 的键**整键丢弃**（递归，数组内的 null 元素也剔除）。
 *
 * 存在的理由：DSH 宿主的输出 schema **不支持 type 数组**（e2e 有硬断言"type 必须是单字符串"），
 * 因此"可能没有值"的数字字段无法写成 `type: ["number","null"]`；而契约校验里
 * `typeOk(null, "number") === false`，直接返回 null 会被判成违反输出契约。
 * 约定：**未知就省略该键**，而不是置 null —— 渲染层用 `?? "?"` 兜底显示。
 */
function dropNullDeep(value) {
  if (Array.isArray(value)) return value.map(dropNullDeep).filter((v) => v !== null && v !== void 0);
  if (value !== null && typeof value === "object") {
    const out = {};
    for (const key of Object.keys(value)) {
      const v = value[key];
      if (v === null || v === void 0) continue;
      out[key] = dropNullDeep(v);
    }
    return out;
  }
  return value;
...
function registerNovelPlot(ctx, config) {
  ctx.tools.register({
    ...
    parameters: {
      ...
      properties: {
        book: { type: "string", description: "书名。" },
        action: { type: "string", enum: ["list", "add", "update", "done", "delete", "scan", "graph"], description: "list=查看；add=登记新伏笔；update=修改；done=标记已回收；delete=删除；scan=扫描章节文本，自动更新伏笔提及章节；graph=跨章结构视图（伏笔埋设跨度 / 人物连续缺席 / 剧情线活跃度 / 时间线顺序 / 大纲对比），只读不写盘。" },
        ...
        // v5.0.0：结构视图可调"连续缺席几章算异常"（默认 5）
        absenceThreshold: { type: "integer", description: "graph 时可选：人物连续缺席多少章算异常（默认 5）。" },
        root: { type: "string", description: "章节库根目录。" }
      },
      required: ["book"],
      additionalProperties: false
    ...
    output: {
      schema: {
        ...
        properties: {
          ...
          message: { type: "string" },
          // v5.0.0：action="graph" 的结构视图字段（与伏笔列表共享同一 schema，故全部可选）。
          // 注意 required 仍是 ["book","action","entries"]——graph 分支会带一个空 entries 以满足它，
          // 这样不必为了一个 action 放松契约检查（放松会让另外 6 个 action 少一层断言）。
            ...
            properties: {
              ...
              rows: {
                ...
                items: {
                  ...
                  required: ["chapter", "number", "present"]
                }
              },
              absences: {
                type: "array",
                items: {
                  ...
                  required: ["name", "from", "to", "length"]
                }
              }
            },
            required: ["characters", "rows", "absences"]
          },
          threadActivity: {
            type: "array",
            items: {
              ...
              required: ["thread", "chapters", "length", "gap"]
            }
          },
          timelineOrder: {
            type: "array",
            items: {
              ...
              properties: {
                ...
                event: { type: "string" },
                chapter: { type: "string" },
                // v5.0.0 修正：number **不在 required 里**。时间线条目可能只登记了
                // 「第 3 天 / 某个事件」而 chapter 写的是无法解析的文本，此时 graph.js
                // 会省略 number 键（契约里"未知值"是省略而非 null）。列为必填会让
                // 这类完全合法的设定表触发宿主契约校验失败。
                number: { type: "number" },
                issue: { type: "string" }
              },
              required: ["day", "event", "chapter", "issue"]
            }
          },
          planVsActual: {
            type: "array",
            items: {
              ...
              required: ["number", "direction", "overlap", "keywords"]
            }
          },
          summary: { type: "string" },
          degraded: { type: "array", items: { type: "string" } }
      ...
      render: (_args, value) => {
        ...
        // v5.0.0：结构视图分支——必须在下方的 value.entries.filter 之前返回（graph 响应不带 entries）
        if (value.action === "graph") {
          const out = [`<path>novels/${value.book}</path>`, "<type>novel-plot-graph</type>", "<content>", "", value.summary];
          const life = Array.isArray(value.plotLifecycle) ? value.plotLifecycle : [];
          if (life.length > 0) {
            out.push("", "未回收伏笔（埋设章 → 已过章数）：");
            for (const p of life) out.push(`  - [${p.id}] ${p.content}（第 ${p.firstChapter ?? "?"} 章埋下，已过 ${p.distance ?? "?"} 章）`);
          }
          const matrix = value.characterMatrix ?? {};
          if (Array.isArray(matrix.absences) && matrix.absences.length > 0) {
            out.push("", "连续缺席：");
            for (const a of matrix.absences) out.push(`  - ${a.name}：第 ${a.from}–${a.to} 章（${a.length} 章未出场）`);
          }
          if (Array.isArray(value.threadActivity) && value.threadActivity.length > 0) {
            out.push("", "剧情线：");
            for (const t of value.threadActivity) out.push(`  - ${t.thread}：活跃章 ${t.chapters.join("、") || "（正文无线索）"}｜跨度 ${t.length}｜最大空档 ${t.gap}`);
          }
          const bad = (Array.isArray(value.timelineOrder) ? value.timelineOrder : []).filter((t) => t.issue);
          if (bad.length > 0) {
            out.push("", "时间线顺序问题：");
            for (const t of bad) out.push(`  - ${t.day}（章 ${t.chapter}）：${t.issue}`);
          }
          if (Array.isArray(value.planVsActual) && value.planVsActual.length > 0) {
            out.push("", "大纲 vs 正文：");
            for (const r of value.planVsActual) out.push(`  - 第 ${r.number} 章 重合率 ${r.overlap}：${r.direction}`);
          }
    ...
    async execute(args, exec) {
      ...
      const book = sanitizeSegment(requiredString(args, "book"), "book");
      const action = ["add", "update", "done", "delete", "scan", "graph"].includes(args?.action) ? args.action : "list";
      const root = resolveRoot(config, args, exec);
      // v5.0.0：结构视图走**只读早返回**——必须留在 withFileTx 写事务与 readPlots（带 v0.8.0 旧位置迁移写盘）
      // 之外，否则"只读"承诺会被打破（graph.js 内部实现了不写盘的伏笔读取）。
      if (action === "graph") {
        const graph = await buildStoryGraph(root, book, { absenceThreshold: args?.absenceThreshold }, exec);
        // entries: [] 只为满足共享 schema 的 required（不放松它，避免削弱另外 6 个 action 的契约断言）；
        // dropNullDeep 丢弃 null 值——宿主 schema 不支持 type 数组，null 会被判成违反输出契约。
        return { ...dropNullDeep(graph), entries: [], action: "graph" };
      }
...
function registerNovelBooks(ctx, config) {
  ctx.tools.register({
    name: "novel_books",
    description: "列出小说章节库中的全部作品（novels 文件夹下的子目录），含章节数与总字数。**只读**。",
    parameters: {
      type: "object",
      properties: {
        root: { type: "string", description: "章节库根目录（含 novels 子目录）。" }
...
function registerNovelChapters(ctx, config) {
  ctx.tools.register({
    name: "novel_chapters",
    description: "列出某部作品的全部章节：章号、标题、字数、行数、更新时间。**只读**；若存在同章号多文件，列表末尾会给出撞号分组提示。",
    parameters: {
      type: "object",
      properties: {
        book: { type: "string", description: "书名（novels 下的子目录名）。" },
    ...
    async execute(args, exec) {
      ...
      for (const chapter of chapters) {
        ...
        result.push({
          ...
          // v5.0.0 修正：文件名没标题时回退读正文一级标题（如工具创建的「第07章.md」），
          // 否则用 novel_new_chapter 的 title 建的章在清单里显示为空标题。
          title: chapter.title || stats.heading || "",
          chars: stats.chars,
          lines: stats.lines,
...
function registerNovelRead(ctx, config) {
  ctx.tools.register({
    name: "novel_read",
    description: "阅读某部作品的某个章节，返回带行号的正文（含字数统计）。可用 offset/limit 分段读取长章节。**只读**：不写任何文件、不改动任何数据。",
    parameters: {
      type: "object",
      properties: {
        book: { type: "string", description: "书名。" },
    ...
    async execute(args, exec) {
      ...
      // v4.3.0：行数口径与 core.js 的 chapterStats 统一（countTextLines：空文件 0 行）。
      // 旧版直接取 split 长度 → 空文件报 1 行，与 novel_chapters 的 0 行自相矛盾。
      // 断言用 Math.max(totalLines,1) 兜底，保证空章节仍可按 offset=1 读到那条空行（行为不变）。
      const totalLines = countTextLines(text);
      assert(offset <= Math.max(totalLines, 1), `offset ${offset} 超出范围（"${chapter.file}" 共 ${totalLines} 行）`);
      let chars = 0;
      const lines = [];
  ...
  ctx.tools.register({
    ...
    async execute(args, exec) {
      ...
      try {
        ...
        // v4.3.0：关键词报告同样按书保留最近 REPORT_CACHE_KEEP 份（缓存键含内容哈希，改稿即新增文件）
        await pruneAnalysisReports(reportDir, book, "keywords");
...
/**
 * v5.0.0：把章节标题清洗成可安全入文件名的片段（novel_new_chapter 用）。
 * 走 sanitizeSegment（剥路径分隔符/Windows 保留字符/首尾点与空格，保留名加下划线），
 * 再限长 24 字——文件名只是给人和 findChapter 用的标识，过长反而难读。
 * 标题全是非法字符时返回空串，退回不带标题的文件名，**不因此拒绝创建章节**。
 */
function safeTitleSegment(title) {
  try {
    return sanitizeSegment(title, "title").slice(0, 24);
  } catch {
    return "";
  }
}
/**
 * v5.0.0（降本）：精简工作流开关。true 时注入 LEAN_TEXT、报告类工具默认走精简输出。
 * 状态字段由 core.js 持久化（缺省/非法值一律 false），这里只做只读判断。
 */
function isLeanWorkflow() {
  try { return readSentenceStateSync()?.leanWorkflow === true; } catch { return false; }
}
function registerNovelNewChapter(ctx, config) {
  ctx.tools.register({
    ...
    async execute(args, exec) {
      ...
      // v5.0.0 修正：把标题带进文件名（本书既有命名约定「第01章 夜航.md」）。
      // 旧实现文件名恒为「第NN章.md」，标题只写进一级标题，而 novel_chapters 的标题取自文件名
      // → 用 title 建的章在清单里显示为空标题；顺带也让 findChapter 的"标题子串"能命中新章。
      const titleSegment = title === void 0 ? "" : safeTitleSegment(title);
      const fileName = `第${String(number).padStart(2, "0")}章${titleSegment === "" ? "" : " " + titleSegment}.md`;
      ...
      const reminder = "本章已创建（第" + String(number).padStart(2, "0") + "章）。【基线怎么用】① 动笔前：anchors 读一遍校准语感即可（**不要照 skeletons 造句**），baseline 六维只做写完后的校验；② 写完：**建议**跑一次 novel_style_check，只有偏离明显（≥2 倍容差）或那一处读起来确实别扭时才逐句修正跑偏处（不得整章重写）——单一维度轻微出带属正常波动，不要为对齐数字改文。";
...
function apply(ctx, config = {}) {
  ...
  // v5.0.0：开写包（动笔前装配材料）——紧跟新建章节，因为它的典型调用时机就是"要写下一章了"
  registerNovelChapterBrief(ctx, config);
  ...
  // v5.0.0：改稿台（诊断转待办 + 复测）——紧跟风格自检，二者是"判定"与"行动"的关系
  registerNovelFixPlan(ctx, config);
  ...
  // v4.3.0：把包内 novel-writing 技能注册给宿主（可选注入——宿主无 skills 服务时静默跳过，不影响其余功能）
  ctx.inject(["skills"], (sctx) => {
    // 服务缺失是合法的宿主配置（headless / 裁剪版），不是错误——照 webServer 的写法静默跳过
    if (!sctx || !sctx.skills || typeof sctx.skills.registerProvider !== "function") return;
    try {
      sctx.skills.registerProvider(() => skillProvider);
    } catch (error) {
      console.warn("[dsh-novel-writer] skill provider registration failed:", error instanceof Error ? error.message : String(error));
    }
  });
  ...
  ctx.systemPrompt.section({
    ...
    text: () => {
      try {
        ...
        // v5.0.0：档位（注入多少）× 场景（注入什么）。选择逻辑收进 prompts.js 的纯函数 promptTextFor——
        // 这里只负责读状态与兜底，便于单测覆盖"老状态文件（无 promptScene）行为不变"。
        // v5.0.0（降本）：leanWorkflow=true 时优先注入精简档（LEAN_TEXT），优先于场景——省 token 比"注入什么"更要紧
        return promptTextFor(st.systemPromptMode, st.promptScene, st.leanWorkflow === true);
      } catch { /* 读不到开关时按精简档处理 */ }
      return BRIEF_TEXT;
    }
  });
