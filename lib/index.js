...
import { analyzeText, CATEGORY_LABELS, CATEGORY_ORDER } from "./analysis.js";
...
async function readSentenceState() {
  try {
    ...
    return { ...SENTENCE_ANALYSIS_DEFAULTS };
  }
}
...
/** 生效配置优先级：UI 开关（state 文件）> 插件 config.sentenceAnalysis > 默认值。 */
function effectiveSentenceAnalysis(config, state) {
  const configured = config?.sentenceAnalysis ?? {};
  return {
    enabled: state?.enabled ?? configured.enabled ?? SENTENCE_ANALYSIS_DEFAULTS.enabled,
    autoAnalyze: state?.autoAnalyze ?? configured.autoAnalyze ?? SENTENCE_ANALYSIS_DEFAULTS.autoAnalyze
  };
}
...
/** /api/dsh-novel-writer/state 路由：GET 读开关，POST 写开关。 */
function makeStateRoutes() {
  ...
  const handler = async (req, res) => {
    ...
    if (method === "GET") {
      ...
      return writeJson(res, 200, { ...state, file: stateFilePath() });
    }
    if (method === "POST") {
  ...
  return [{ kind: "exact", path: routePath, handler }];
}

/** 句式类型短码（渲染用）。 */
const TYPE_CODE = { statement: "陈述", dialogue: "对话", psychology: "心理", question: "疑问", "rhetoric-question": "反问", exclamation: "感叹", imperative: "祈使", ellipsis: "省略" };
...
（可用 novel_sentence_config 查看/开启）
  ...
  lines.push(`- 对话占比 ${(value.style.dialogueRatio * 100).toFixed(1)}%, 心理占比 ${(value.style.psychologyRatio * 100).toFixed(1)}%, 反问+疑问 ${(value.style.questionRatio * 100).toFixed(1)}%, 感叹 ${(value.style.exclamationRatio * 100).toFixed(1)}%`);
  ...
  lines.push("", "【AI 解读提示】");
  lines.push("结合上述统计与 novel_keywords 词汇偏好，说明作者的句式习惯（对话驱动/心理驱动、长短句节奏、反问或感叹的修辞倾向）、");
  lines.push("句式排列规律（如“对话→陈述→心理”循环），以及文中流露的主观情感倾向，并给出具体例证。");
  lines.push("", "</content>");
  return lines.join("\n");
}
...
function registerNovelSentenceAnalysis(ctx, config) {
  ctx.tools.register({
    ...
    description: "句式模式分析：统计某部作品（或单章）的句式分布（陈述/对话/心理/疑问/反问/感叹/祈使/省略留白）、句式排列规律（转移与高频模板）、段落结构、句长分布、情感曲线与风格指纹，用于快速掌握作者的写作习惯与主观情感特征。受 UI 开关控制，可先用 novel_sentence_config 查看状态。",
    parameters: {
      type: "object",
      properties: {
      ...
      schema: {
        ...
        properties: {
          ...
          style: {
            ...
            required: ["dialogueRatio", "psychologyRatio", "questionRatio", "exclamationRatio", "shortSentenceRatio", "longSentenceRatio", "subjectivityIndex", "emotionDensity", "avgSentenceLength", "firstPersonDensity"]
          },
          emotion: {
            type: "object",
            additionalProperties: false,
    ...
    async execute(args, exec) {
      ...
      const top = optionalInt(args, "top", 1, 20, 8);
  ...
  ctx.tools.register({
    ...
    parameters: {
      ...
      properties: {
        ...
        enabled: { type: "boolean", description: "set 时指定：是否启用句式模式分析。" },
    ...
    output: {
      schema: {
        ...
        required: ["file", "enabled", "autoAnalyze"]
      },
      render: (_args, value) => [{
        type: "text",
...
${value.updated === true ? "(已保存到 state 文件)" : ""}
    ...
    async execute(args) {
      ...
      const hasState = current.enabled !== void 0 || current.autoAnalyze !== void 0;
      const source = hasState ? "state 文件（GUI 开关）" : (config?.sentenceAnalysis ? "插件 config" : "默认值");
      return { file: stateFilePath(), enabled: after.enabled, autoAnalyze: after.autoAnalyze, source, updated };
    }
  });
}
...
const WORKFLOW_TEXT = [
  ...
  "4. 句式模式分析（增强，受 UI 开关控制）：分析作品时可调用 novel_sentence_analysis 提取句式分布（陈述/对话/心理/疑问/反问/感叹/祈使/省略留白）、",
  "   句式排列规律（转移、高频模板、段首段尾句式）、句长节奏、情感曲线与风格指纹，用于快速掌握作者的写作习惯与主观情感。",
  "   开关默认开启；可在 Web GUI 侧边栏「句式分析」面板或 novel_sentence_config 中调整。动手前可先调用 novel_sentence_config 查看状态：",
  "   - enabled=false 时 novel_sentence_analysis 会返回关闭状态，不要强行分析；",
  "   - autoAnalyze=true 时分析作品应主动附带句式分析；false 时仅在用户明确要求时调用。",
  "5. 续写 / 辅助写作时：先通读最近的章节（保持人物、时间线、情节伏笔、文风与已提炼的关键词一致），再动笔。",
  "   新章节写入 novels/<书名>/ 文件夹：用 novel_new_chapter 创建章节文件，或直接用 write 工具写文件。",
...
function apply(ctx, config = {}) {
  ...
  // 注册 UI 开关状态路由（inject 已声明 webServer；保留兜底兼容非标准宿主）
  const webServer = ctx.webServer ?? ctx.get?.("webServer");
  if (webServer) {
    const routes = makeStateRoutes();
    ctx.effect?.(() => {
