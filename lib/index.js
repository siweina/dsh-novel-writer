/**
 * dsh-novel-writer — 小说写作助手插件（v3.9.5：render 全量可见化 / 锚包写作 / 对照修正 / 缺陷整修）
...
async function openInExplorer(dir) {
  if (process.platform === "win32") {
    ...
    try { execFile("open", [dir], function () {}); return true; } catch { return false; }
  }
  try { execFile("xdg-open", [dir], function () {}); return true; } catch { return false; }
}
...
async function bookAnalysisCached(root, keyName, text, featState) {
  ...
  if (cached && cached.th === th && cached.analysis && cached.ver === CACHE_VERSION) {
    ...
    if (recomputed) {
      try { await atomicWriteJson(file, { th, analysis: cached.analysis, detection: det, ver: CACHE_VERSION }); } catch { /* 写失败不影响 */ }
    }
...
function registerNovelStyleReport(ctx, config) {
  ctx.tools.register({
    ...
    async execute(args, exec) {
      ...
      if (briefMode && result.baseline) {
        // v3.9.1 优化：按 METRIC_ORDER 顺序输出、用中文标签（与完整报告一致）
        const dims = METRIC_ORDER.filter(function (k) { return result.baseline[k] && typeof result.baseline[k].mu === "number"; });
...
async function detectChapterBridge(root, book, prevFile, currFile, exec) {
  ...
  try {
    ...
    try {
      const embMod = await import("./embedding.js");
      const [vPrev, vCurr] = await Promise.all([embMod.embed(prevTail), embMod.embed(currHead)]);
      if (Array.isArray(vPrev) && Array.isArray(vCurr)) {
        const sim = embMod.cosine(vPrev, vCurr);
    ...
    async execute(args, exec) {
        ...
        for (const name of Object.keys(per)) {
          ...
          for (const item of list) {
            if (Math.abs(item.val - mu) > thr) {
              out.push({ type: "OOC·情绪偏离", detail: "角色「" + name + "」在 " + item.file + " 的情绪值 " + item.val.toFixed(2) + " 偏离其全书均值 " + mu.toFixed(2) + "（阈值 " + (thr === 0.3 ? "小样本固定 0.3" : "全书 2σ") + "）——若有重大剧情触发情绪变化可忽略，否则可能 OOC", chapters: [item.file] });
            }
          }
        }
      ...
      if (speech && Array.isArray(speech.honorBad)) {
        ...
        for (const [word, files] of honorHits) {
          ...
          candidates.push({ type: "语用冲突·客套", detail: `「${word}」×${files.length}章 属于「${cultureName}」禁用表达${rec}`, chapters: files }); // v3.7.0 ⑥：按实际词表文化标注
        }
      }
