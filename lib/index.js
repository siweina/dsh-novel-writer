...
function makeStateRoutes(allowLan, config) {
  ...
  const handler = async (req, res) => {
    ...
    if (method === "GET") {
      const state = await readSentenceState();
      const plotsRoot = config?.root ?? state.lastRoot;
      return writeJson(res, 200, {
        enabled: state.enabled,
        autoAnalyze: state.autoAnalyze,
        tools: state.tools ?? {},
        plotsDir: typeof plotsRoot === "string" && plotsRoot.length > 0 ? join(plotsRoot, ".novel-writer") : "",
        file: stateFilePath()
      });
    }
    if (method === "POST") {
  ...
  const revealHandler = async (req, res) => {
    ...
    if (target !== "plots-dir") return writeJson(res, 400, { error: 'unknown target (allowed: "plots-dir")' });
    const state = await readSentenceState();
    const plotsRoot = config?.root ?? state.lastRoot;
    if (typeof plotsRoot !== "string" || plotsRoot.length === 0) {
      return writeJson(res, 404, { error: "书库根未知：先在对话中调用一次 novel_plot（或配置插件 root），即可在 UI 中打开伏笔文件夹。" });
    }
    const dir = join(plotsRoot, ".novel-writer");
    try {
...
function normalizePlotEntry(entry) {
  ...
  if (entry.chapter !== void 0 && entry.chapter !== null) out.chapter = String(entry.chapter);
  if (entry.note !== void 0 && entry.note !== null) out.note = String(entry.note);
