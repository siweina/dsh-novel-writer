...
function registerNovelSentenceConfig(ctx, config) {
  ctx.tools.register({
    ...
    description: "查看或修改句式模式分析的开关状态：enabled（分析功能是否可用）、autoAnalyze（分析作品时是否主动使用）、systemPromptMode（系统提示词档位 off/brief/full）与 promptScene（提示词场景 general/writing/revising/auditing/setup，决定完整档下注入哪一套流程）。",
    parameters: {
      type: "object",
      properties: {
  ...
  ctx.systemPrompt.section({
    ...
    text: () => {
      try {
        ...
        return promptTextFor(st.systemPromptMode, st.promptScene);
      } catch { /* 读不到开关时按精简档处理 */ }
      return BRIEF_TEXT;
    }
  });
