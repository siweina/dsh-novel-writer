...
const FEATURE_DEFAULTS = Object.freeze({ emotionCaveat: true, genreTheme: true, emotionComplexity: true });
    ...
    async execute(args) {
      ...
      if (action === "set") {
        ...
        if (args?.features !== null && typeof args?.features === "object") {
          const featPatch = {};
          for (const name of ["emotionCaveat", "genreTheme", "emotionComplexity"]) {
...
const WORKFLOW_TEXT = [
  ...
  "   若发现异名同书（如\"大梁秘史/大梁风云\"实为同一本），用 book 参数强制合并后以 apply 模式导入到 novels/<书名>/ 分类存放。",
