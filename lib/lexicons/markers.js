...
const SPEECH_STYLE_RULES = {
  western: {
    ...
    // v3.9.1 M22：数字支持"两"；量词兼容"炷"（正字）与"柱"（笔误文本）；"上香"本身仍命中
    ritualBadPatterns: [/上[一二三四五六七八九十百千两]*[柱炷]?香/g, /[一二三四五六七八九十百千两]+[柱炷]香/g, /烧香/g, /磕头/g, /跪拜/g],
...
const CULTURE_MARKERS = {
  western: {
    label: "西方/欧式中世纪",
    // v4.3.0 卫生修正：删除表内重复词条（"城堡"在尾部二次出现）——scanWordHits 的占用位图已保证不双计，
    // 去重只为词表可读性；首个出现位置保留，其余词的相对顺序不变，detectCulture 结果不受影响。
    words: ["教堂","神甫","神官","公爵","伯爵","骑士","城堡","教廷","祈祷","礼拜","弥撒","修士","修道院","蜡烛","烛台","绞刑架","马车","庄园","国王","王后","领主","圣器","圣水","神像","火刑","审问","燧发","火枪","决斗","舞会","礼服","女仆","男仆","面包","葡萄酒","钟楼","教典","贵族","审判庭"]
  },
  eastern: {
    label: "东方/中式古代",
  ...
  modern: {
    label: "现代都市",
    // v4.3.0 卫生修正：删除表内重复词条（"便利店"/"朋友圈"/"点赞"各二次出现）——同上，去重不改变扫描语义（占用位图不双计）
    words: ["学校","教室","宿舍","操场","体育馆","手机","电视","电脑","网络","微信","QQ","地铁","公交","写字楼","办公室","公司","老板","同事","上班","加班","外卖","快递","便利店","超市","网购","直播","短视频","弹幕","游戏","键盘","耳机","充电","扫码","闸机","霓虹","红绿灯","出租车","电梯","空调","冰箱","沙发","客厅","校服","教师","班主任","学生","社团","食堂","图书馆","课桌","黑板","讲台","考试","成绩","家长","医院","医生","护士","警察","派出所","银行","信用卡","高铁","飞机","航班","工资","房租","房贷","朋友圈","点赞","微博","抖音","贴吧","论坛","网友","内卷","躺平","社恐","打工人","程序员","运营","甲方","乙方","简历","面试","会议室","打印机","蛋糕店","奶茶店","电影院","KTV","健身房","酒吧","咖啡馆","商场","餐厅","宾馆","酒店","电动车","外卖员","快递员","保安","门卫","物业","业主","房东","合同","发票","报销","年会","团建","出差","会议","PPT","Excel","邮件","邮箱","密码","软件","App","打车","拼车","滴滴","淘宝","京东","拼多多","支付宝","支付","红包","转账","照片","视频","相机","自拍","热搜","头条","博主","网红","粉丝","评论","转发","下载","安装","更新","系统","桌面","文件","文件夹","报告","方案","项目","进度","截止","deadline"]
  }
};
