...
const WEB_NOVEL_SIGNALS = {
  ...
  tearjerker: ["虐", "心碎", "对不起", "眼泪", "分手", "绝望", "崩溃★", "哽咽", "追妻★", "火葬场★", "求你", "别走", "疼", "悔婚★", "虐心★", "决绝★", "卑微★", "挽回★", "拒绝★", "替身★"],
  blaze: ["冲", "杀", "斩", "轰", "燃", "战意", "怒吼", "碾压", "横扫", "秒杀★", "一招", "破", "震", "吞噬★", "越级★", "击杀★", "暴涨★", "大比★", "秘境★", "夺宝★", "宗门★", "练气★", "筑基★", "突破", "绝技★", "凶兽★", "宝物★", "金手指★", "天才★", "觉醒★", "传承★", "武道★", "念力★", "雷电★", "火焰★", "超能力★", "医术★", "体质★", "戒指★", "老爷爷★", "重生★", "签到★"],
...
export function computeVibe(detect, emotion, text = "") {
  ...
  if (text) {
    for (const [axis, words] of Object.entries(WEB_NOVEL_SIGNALS)) {
      ...
      for (const w of words) {
        let i = 0;
        while ((i = text.indexOf(w, i)) !== -1) { n += 1; i += w.length; }
      }
      webNovelHits[axis] = n;
    }
  }
  ...
  // ⑧ 热血激昂
  const fightWords = freqIn([...(ev.modern || []), ...(ev.western || []), ...(ev.eastern || [])], ["战", "刀", "剑", "冲", "杀", "拳"]);
  ...
  if ((webNovelHits.absurd || 0) > 0 || (axes.absurd?.weight ?? 0) > 0) push("absurd", Math.min(1, V * 3), 0.4, "情绪跳跃(波动大)");
