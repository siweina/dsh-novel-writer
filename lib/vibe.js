...
const WEB_NOVEL_SIGNALS = {
  ...
  fluff: ["宝贝", "老婆", "亲爱的", "宠溺", "宠", "哄", "撒娇", "搂", "吻", "抱紧", "摸头", "甜", "心头一软", "小傻瓜", "乖乖", "甜宠★", "拉扯★", "白月光★"],
  ...
  if (text) {
    for (const run of String(text).match(/[\u4e00-\u9fa5]{2,}/g) || []) {
      for (let i = 0; i < run.length - 1; i++) {
        const w2 = run.slice(i, i + 2);
        textWordCounts.set(w2, (textWordCounts.get(w2) || 0) + 1);
        if (i + 2 < run.length) {
          const w3 = run.slice(i, i + 3);
          textWordCounts.set(w3, (textWordCounts.get(w3) || 0) + 1);
        }
        if (i + 3 < run.length) {
          const w4 = run.slice(i, i + 4);
          textWordCounts.set(w4, (textWordCounts.get(w4) || 0) + 1);
        }
      }
    }
  }
