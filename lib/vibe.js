...
import { IMPLICIT_EMOTION_PROTOTYPES } from "./embedding.js";
...
const WEB_NOVEL_SIGNALS = {
  ...
  mystery: ["线索", "真相", "谜团", "疑点", "秘密", "调查", "发现", "蛛丝马迹★", "身份反转★", "震撼★"],
...
export function computeVibe(detect, emotion, text = "") {
  ...
  const push = (axis, score, weight, label, isEvidence) => {
    axes[axis] = axes[axis] || { score: 0, weight: 0, evidenceScore: 0, evidenceWeight: 0, signals: [] };
  ...
  const linkThemes = (axis) => {
    for (const l of THEME_AXIS_LINK) {
      if (l.axis === axis && l.themes.some((t) => themes.includes(t) || theme.includes(t))) {
        ...
        if (evidenceBase(axis) >= l.needBase) push(axis, l.bonus, 1.2, "题材联动:" + l.themes[0]);
        return;
      }
    }
  };
