...
function isImperative(text) {
  const starts = "^(你|您|我们|咱们|大家|喂)?(" + IMPERATIVE_HARD.sort(function (a, b) { return b.length - a.length; }).join("|") + ")";
  if (new RegExp(starts).test(text)) return true;
  const verbRe = new RegExp("^(你|您|我们|咱们|大家|喂)?(" + IMPERATIVE_VERBS.join("|") + ")(吧|[。！？!?…]|$)");
 ...
 */
const NEGATION_PREFIXES = ["不要", "不用", "不必", "不再", "不是", "并非", "绝非", "毫不", "丝毫不", "从不", "从未", "没有", "没啥", "未曾", "无法", "难以"];
...
function negatedAt(text, idx) {
  ...
  if (NEGATION_SINGLE.has(text[idx - 1])) return true;
...
function emotionOf(text) {
  ...
  for (const [emotion, list] of Object.entries(EMOTION_WORDS)) {
    for (const word of list) {
      ...
      while (from < text.length) {
        ...
        if (negatedAt(text, idx)) { from = idx + word.length; continue; }
  ...
  try {
    ...
    for (const w of new Set(bigrams)) {
      ...
      // v3.9.1 #5：逐次 indexOf 循环——每次命中处都做 negatedAt 过滤，按通过次数计 nOcc（不再"只过滤首次+按全文出现次数"）
      let nOccur = 0;
      let hitPos = text.indexOf(w);
      while (hitPos !== -1) {
        if (!negatedAt(text, hitPos)) nOccur += 1;
        hitPos = text.indexOf(w, hitPos + w.length);
      }
      if (nOccur === 0) continue;
      scores[emo] += nOccur; // v3.7.0 ④：按通过过滤的出现次数（与主词表口径一致）
...
export function implicitEmotionScan(text, semResolver = null) {
  ...
  const see = (label, word, n, valence) => {
    ...
    else posHits += n * valence; // v3.9.1 #9：移除 if(false) fragileHits 死分支（保留 else 逻辑）
  ...
  return {
    ...
    // v3.9.1 #11：透出内部实际使用的加权计数（与上面比率计算同源；比率口径不变）
    negHits,
    posHits,
    ambHits,
    ambiguous,
...
export function analyzeText(text, options = {}) {
  ...
  if (truncated) {
    guidance += "\n⚠️ 文本超过采样上限(" + maxSentences + " 句),以上统计基于前缀采样;建议分章分析。";
  }
