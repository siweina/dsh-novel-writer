...
export function splitBlocks(text) {
  ...
  for (const line of lines) {
    ...
    if (!hasBlank) {
      // v4.0.0 修正：去掉恒真的标题判断（138 行已 continue，标题走不到这里）
      if (!/^[-_*=]{3,}$/.test(trimmed)) blocks.push(trimmed);
      continue;
    }
...
function isQuestion(text) {
  ...
  if (/(吗|呢|嘛)$/.test(text)) return true;
...
function isImperative(text) {
  ...
  if (hardSingle.length > 0) {
    ...
    if (singleRe.test(text)) return true;
  }
...
const BIE_NON_NEGATION_PREFIX = new Set(["特", "个", "分", "差", "告", "判", "识", "辨", "离", "级", "性", "类", "区", "派", "辞", "阔", "作", "待", "辈", "隔", "吻"]);
 ...
 * 口径与 emotionOf 的 cleanScores 一致：只扫强词表 + 否定过滤 + 副词加权。
 */
function cleanScoresOf(text) {
  const counts = { joy: 0, anger: 0, sorrow: 0, fear: 0, surprise: 0 };
  for (const [emotion, list] of Object.entries(STRONG_EMOTION_WORDS)) {
    for (const word of new Set(list)) {
      let from = 0;
      while (from < text.length) {
        const idx = text.indexOf(word, from);
        if (idx === -1) break;
        if (!negatedAt(text, idx)) counts[emotion] += adverbWeight(text, idx);
...
function emotionOf(text) {
  ...
  const emotionOrder = new Map(Object.keys(EMOTION_WORDS).map((name, index) => [name, index]));
  const occurrences = [];
  ...
  for (const occ of occurrences) {
    ...
    if (STRONG_WORD_KEYS.has(occ.emotion + ":" + occ.word)) cleanScores[occ.emotion] += weight;
  ...
  // 性能安全：只查"文本中出现的 2-4 字片段"（O(句长×3)），不做 27k 词全表扫描
  try {
    ...
    for (let bi = 0; bi < text.length; bi += 1) {
      ...
      for (let len = Math.min(4, text.length - bi); len >= 2; len -= 1) {
...
export function implicitEmotionScan(text) {
  ...
  let negWeight = 0, posWeight = 0;
  const carrierCounts = new Map();
  const ambiguous = [];
  const see = (label, word, valence) => {
    if (valence < 0) { negCount += 1; negWeight += -valence; }
    else { posCount += 1; posWeight += valence; }
  ...
  return {
    negative: total === 0 ? 0 : Math.round((negCount / total) * 100) / 100,
    positive: total === 0 ? 0 : Math.round((posCount / total) * 100) / 100,
    ...
    posHits: posCount,
    ambHits: ambCount,
    weightedTotal: Math.round((negWeight + posWeight) * 1000) / 1000,
    ambiguous,
    topCarriers: top
  };
}
...
export function valenceSeries(text, winChars = 100) {
  ...
  return { series, seriesFull, posWords, negWords, windowCount: windowTotal, hitWindowCount: series.length, windowPosNeg };
}
...
export function valenceStats(text) {
  ...
  const { series, seriesFull, posWords, negWords, windowPosNeg } = valenceSeries(text);
  const n = series.length;
  const windowTotal = windowPosNeg.length;
  // v3.7.0：零命中窗口跳过后的空系列——补全字段（下游 emotion.quantification.stats 恒有值）
  if (n === 0) return { windows: windowTotal, hitWindows: 0, variance: 0, adjVariance: 0, delta: 0, deltaRobust: 0, conflict: 0, posRatio: 0, negRatio: 0, meanValence: 0 };
  ...
  let adjSum = 0;
  for (let i = 1; i < m; i += 1) adjSum += Math.abs(seriesFull[i] - seriesFull[i - 1]);
  const adjVariance = m > 1 ? adjSum / (m - 1) : 0;
...
export function analyzeText(text, options = {}) {
  ...
  // 现在词频类统计一律以"采样文本"为样本；totalChars 仍保留全文长度（顶层字段语义不变）。
  // 直接拼接句子（句内已含句末标点，不会跨句造词），避免 join 分隔符虚增分母。
  const sampleText = truncated ? sentences.map((s) => s.text).join("") : text;
  const sampleChars = truncated ? sampleText.length : totalChars;
  ...
  for (const sentence of sentences) {
    ...
    for (const [emotion, words] of Object.entries(sentence.emotion.words)) {
      for (const word of words) {
        ...
        // v4.0.0 修正：与 scores 同口径——被否定（"不害怕"）的命中不再计入词频（旧版 split 原始计数与 scores 打架）
        let nInSentence = 0;
        const stx = sentence.text || "";
        let from = 0;
        while (from < stx.length) {
          const hit = stx.indexOf(word, from);
          if (hit === -1) break;
          if (!negatedAt(stx, hit)) nInSentence += 1;
          from = hit + word.length;
        }
        if (nInSentence === 0) continue;
