...
| ② **动笔** | 「写第 N 章」 | `novel_new_chapter` · `write` | 新章文件（自动附基线 μ 摘要与原著锚包）；照锚段的味道与骨架写，数字只做事后校验 |
| ③ **自检** | 「自检这一章」 | `novel_style_check` · `novel_sentence_analysis` | 相似度 + 偏差清单 + 六维对照（带内 ✓ / 出带 ⚠）+ 出带处的原著锚段（供逐句对照修正，不整章重写） |
| ④ **改稿** | 「把这章排成改稿清单」 | `novel_fix_plan` | 按「严重度 → 难度 → 行号」排序的待办，每条带原句行号、当前值/目标值、锚段与改写方向；改完 `verify` 三态复测（已解决/仍存在/新检出）、`mark` 登记处置 |
...
9. **全工具 UI 开关**：侧边栏「写作助手功能」面板（总开关 + 工具开关分组 + 功能开关），大白话文案，显示数据目录占用与语义引擎状态。
